...
run_check "unallocated_historical_floor_adjustment" "warning" \
  "WITH pool_events AS (SELECT operation.\"CreatedAtUtc\", operation.\"Id\", CASE WHEN operation.\"OperationKind\" = 'income' THEN operation.\"Amount\" ELSE -operation.\"Amount\" END AS delta FROM financial_operations operation WHERE NOT operation.\"IsCanceled\" AND operation.\"OperationKind\" IN ('income', 'expense') AND NOT EXISTS (SELECT 1 FROM fund_operations fund_operation WHERE fund_operation.\"SourceFinancialOperationId\" = operation.\"Id\" AND NOT fund_operation.\"IsCanceled\") UNION ALL SELECT operation.\"CreatedAtUtc\", operation.\"Id\", CASE WHEN operation.\"OperationKind\" = 'withdraw' THEN operation.\"Amount\" ELSE -operation.\"Amount\" END FROM fund_operations operation WHERE NOT operation.\"IsCanceled\" AND operation.\"SourceFinancialOperationId\" IS NULL UNION ALL SELECT operation.\"CreatedAtUtc\", operation.\"Id\", CASE WHEN operation.\"Direction\" = 'increase' THEN operation.\"Amount\" ELSE -operation.\"Amount\" END FROM cash_bank_balance_operations operation), running_pool AS (SELECT delta, sum(delta) OVER (ORDER BY \"CreatedAtUtc\", \"Id\") AS running_total FROM pool_events), balances AS (SELECT coalesce(sum(delta), 0) AS accounting_total, greatest(coalesce(sum(delta), 0) - least(coalesce(min(running_total), 0), 0), 0) AS available_total FROM running_pool) SELECT CASE WHEN round(accounting_total, 2) = round(available_total, 2) THEN 0 ELSE 1 END FROM balances;"
run_check "cash_bank_fund_reconciliation_mismatch" "critical" \
  "WITH financial AS (SELECT coalesce(sum(CASE WHEN \"OperationKind\" = 'income' THEN \"Amount\" ELSE -\"Amount\" END), 0) AS total FROM financial_operations WHERE NOT \"IsCanceled\" AND \"OperationKind\" IN ('income', 'expense')), adjustments AS (SELECT coalesce(sum(CASE WHEN \"Direction\" = 'increase' THEN \"Amount\" ELSE -\"Amount\" END), 0) AS total FROM cash_bank_balance_operations), named AS (SELECT coalesce(sum(\"Balance\"), 0) AS total FROM funds WHERE NOT \"IsArchived\"), pool_events AS (SELECT CASE WHEN operation.\"OperationKind\" = 'income' THEN operation.\"Amount\" ELSE -operation.\"Amount\" END AS delta FROM financial_operations operation WHERE NOT operation.\"IsCanceled\" AND operation.\"OperationKind\" IN ('income', 'expense') AND NOT EXISTS (SELECT 1 FROM fund_operations fund_operation WHERE fund_operation.\"SourceFinancialOperationId\" = operation.\"Id\" AND NOT fund_operation.\"IsCanceled\") UNION ALL SELECT CASE WHEN operation.\"OperationKind\" = 'withdraw' THEN operation.\"Amount\" ELSE -operation.\"Amount\" END FROM fund_operations operation WHERE NOT operation.\"IsCanceled\" AND operation.\"SourceFinancialOperationId\" IS NULL UNION ALL SELECT CASE WHEN operation.\"Direction\" = 'increase' THEN operation.\"Amount\" ELSE -operation.\"Amount\" END FROM cash_bank_balance_operations operation), accounting_pool AS (SELECT round(coalesce(sum(delta), 0), 2) AS total FROM pool_events) SELECT CASE WHEN round(financial.total + adjustments.total - named.total - accounting_pool.total, 2) = 0 THEN 0 ELSE 1 END FROM financial CROSS JOIN adjustments CROSS JOIN named CROSS JOIN accounting_pool;"
