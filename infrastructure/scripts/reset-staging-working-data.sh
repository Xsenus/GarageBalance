...
EXPECTED_CONFIRMATION="RESET GARAGEBALANCE STAGING"

archive_path="${1:-}"
confirmation="${2:-}"
timestamp="$(date +%Y%m%d-%H%M%S)"
work_dir="${APP_ROOT}/reset-${timestamp}-$$"
backup_file="${APP_ROOT}/backups/working_data_before_${timestamp}.pgdump"
...
restore_on_error() {
  local exit_code=$?
  trap - ERR
  log "stagingResetStatus=failed; exitCode=${exit_code}"
  if [[ "$mutation_started" == "1" && -s "$backup_file" && -n "$database_name" ]]; then
    ...
    log "stagingResetRollbackStatus=completed; backup=${backup_file}"
  fi
...
[[ "$(id -u)" == "0" ]] || { log "stagingResetStatus=refused; reason=root-required"; exit 64; }
[[ "$confirmation" == "$EXPECTED_CONFIRMATION" ]] || { log "stagingResetStatus=refused; reason=confirmation"; exit 64; }
[[ -n "${SUDO_USER:-}" && "$SUDO_USER" != "root" ]] || {
  log "stagingResetStatus=refused; reason=sudo-user"
  exit 64
}
expected_archive_directory="/home/${SUDO_USER}"
archive_directory="$(dirname -- "$archive_path")"
archive_name="$(basename -- "$archive_path")"
[[ "$archive_directory" == "$expected_archive_directory" && "$archive_name" =~ ^reset-[0-9]+\.tar\.gz$ ]] || {
  log "stagingResetStatus=refused; reason=archive-path"
  exit 64
}
[[ -f "$archive_path" && ! -L "$archive_path" && -s "$archive_path" ]] || {
  log "stagingResetStatus=refused; reason=archive-missing"
  exit 66
}
[[ "$(stat -c '%U' "$archive_path")" == "$SUDO_USER" ]] || {
  log "stagingResetStatus=refused; reason=archive-owner"
  exit 64
}
[[ -f "$ENV_FILE" ]] || { log "stagingResetStatus=refused; reason=environment-missing"; exit 66; }
...
[[ "$database_name" == "garagebalance_staging" ]] || {
  log "stagingResetStatus=refused; reason=database; database=${database_name:-missing}"
  exit 64
}
reset_connection="Host=/var/run/postgresql;Database=${database_name};Username=postgres"
...
[[ -x "${work_dir}/GarageBalance.ShowcaseSeed" ]] || {
  log "stagingResetStatus=refused; reason=runner-missing"
  exit 66
}

log "stagingResetBackupStatus=started; database=${database_name}"
sudo -u postgres pg_dump --format=custom "$database_name" > "$backup_file"
[[ -s "$backup_file" ]]
chown "$APP_USER:$APP_GROUP" "$backup_file"
chmod 600 "$backup_file"

restore_check_database="garagebalance_reset_restore_${timestamp//-/}_$$"
...
restore_check_database=""
log "stagingResetBackupStatus=verified; tables=${restored_tables}; file=${backup_file}"

systemctl stop "$SERVICE_NAME"
service_stopped=1
mutation_started=1
log "stagingResetStatus=running"
sudo -u postgres env \
  GARAGEBALANCE_SHOWCASE_CONNECTION="$reset_connection" \
  GARAGEBALANCE_SHOWCASE_CONFIRMATION="$EXPECTED_CONFIRMATION" \
  "${work_dir}/GarageBalance.ShowcaseSeed" reset
...
log "stagingResetStatus=ok; database=${database_name}; backup=${backup_file}"
