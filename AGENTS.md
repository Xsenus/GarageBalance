...
All user-deliverable screenshots of the application interface must be captured with the browser viewport set to exactly `1366×768` CSS pixels at 100% zoom so the target layout is unambiguous. Do not send screenshots from another viewport unless the user explicitly asks for an additional size.
...
Create active roadmaps in `docs/roadmaps/` only when the user explicitly asks for a roadmap or when a large multi-stage task genuinely needs a persistent execution plan. Before starting work tied to an active roadmap, open it and update statuses as the work progresses.

Completed roadmaps and one-off verification logs are not permanent project documentation. Preserve their history in Git and user-facing changes in release notes instead of accumulating frozen copies in `docs/`.
...
When the user asks to create or prepare a roadmap, make it a full execution roadmap, not a short plan. It must be checkable by the same status markers, broken into backend, frontend, data/migrations, tests, documentation, deployment, and acceptance work where relevant. Store a persistent roadmap in `docs/roadmaps/`. Future agents must be able to open it, continue from the current statuses, and update each item as work is completed.

Always update `История выполнения` in the relevant active roadmap when you complete, verify, defer, or unblock meaningful roadmap work. If no active roadmap exists, do not create one solely to log the task.

When reporting an active roadmap status to the user, include a percentage summary: how many checkable items are completed, in progress, not started, blocked, waiting for decisions, and waiting for acceptance. Calculate the percentage only from the active roadmap checklist and exclude `История выполнения` entries so historical records do not inflate progress.
...
Use one shared interaction pattern for prominent creation buttons such as `Добавить гараж`, `Добавить поставщика`, `Добавить отдел`, `Добавить сотрудника`, `Добавить начисление`, and similar actions. These controls must use the shared `create-action-button` class so hover, keyboard focus, active, disabled, and `prefers-reduced-motion` behavior stays identical throughout the application. Every such button must include a meaningful icon from the established icon library that describes the created object or operation; do not use a text `+` or the generic `Plus` icon as a substitute for a semantic icon. The mathematical plus icon may remain only in compact increment/replenishment controls where `+` is the operation itself. Do not introduce feature-specific hover animation for creation buttons when the shared pattern is sufficient.
