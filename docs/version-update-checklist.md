...
### Expand/contract для обратимого deploy

- [ ] В первом (expand) релизе добавлять новые таблицы/колонки/индексы как совместимые и nullable либо с безопасным значением по умолчанию; старые объекты пока не удалять и не переименовывать.
- [ ] Развернуть код, который умеет читать новую схему, но сохраняет совместимость с предыдущим бинарником.
- [ ] Перенести данные отдельной наблюдаемой операцией и проверить старый и новый сценарии.
- [ ] Удалять прежний объект только в отдельном (contract) релизе после успешной приемки и истечения окна rollback предыдущего бинарника.
- [ ] Не объединять несовместимые expand и contract в один deploy. Исключение возможно только с отдельным согласованным окном простоя и проверенным восстановлением предрелизного `pg_dump`.
...
- [ ] Убедиться, что при ошибке после начала миграций apply-скрипт пишет `databaseRollbackStatus=completed`: он останавливает сервис, восстанавливает предрелизный dump и только затем запускает предыдущий бинарник.
...
- [ ] Для автоматического VPS-deploy не выполнять ручное частичное восстановление поверх apply-скрипта: он атомарно возвращает и БД, и каталоги предыдущего релиза.
