...
- [x] Утверждено 17.07.2026: назначения `Прочие оплаты` и `Прочие доходы` имеют стабильные системные коды `other_payments` и `other_income`, явную связь с фондами назначения и не допускают двойного распределения одного поступления.
...
- [x] Stable destinations and fund routing: migrations создают и нормализуют `other_payments`/`other_income`, а PostgreSQL integration-тесты проверяют маршрутизацию, отмену, восстановление и связь с фондом.
