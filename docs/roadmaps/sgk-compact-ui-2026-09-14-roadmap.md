...
- [x] По уточнению заказчика от 15.09.2026 реализовать в карточке гаража годовые платежи как реальные поступления с распределением по начислению, фонду, кассе/общему пулу и аудиту.
...
- 2026-09-15 — Завершено уточнение по годовым платежам из карточки гаража. По выбранному году видны услуга, связанный фонд либо общий пул, начислено, оплачено, остаток и статус; бухгалтер с правом записи рассчитывает недостающие обязательства и проводит остаток как обычное поступление. Сервер привязывает его к конкретному годовому начислению, отражает в кассе/банке, общем пуле, фонде и аудите, запрещает повторную оплату и переплату. Годовое начисление не зависит от месяца регистрации гаража. Двойной щелчок унифицирован для остальных редактируемых таблиц и не перехватывает вложенные действия.
- 2026-09-15 — Финальный gate годовых платежей успешен. Backend: `2898/2898` тестов без пропусков на PostgreSQL 17, строки `91,27%`, ветви `76,43%`; frontend: `109/109` файлов, `1273/1273` теста, statements `88,62%`, branches `81,60%`, functions `87,01%`, lines `89,80%`. Прошли Release build без предупреждений, format, privacy, аудит пакетов, lint, production build, npm audit, Docker distribution и bundle `297450/297984` байт. Idempotent SQL из 147 миграций дважды применён к чистой базе, pending model changes отсутствуют. При `1366×768` карточка открылась двойным кликом, document остался `1366/1366`, три обязательства рассчитались на 2 000 руб., платеж 300 руб. получил статус «Оплачен»; PostgreSQL подтвердила целевое распределение и фондовую проводку, browser console чиста.
- 2026-09-15 — Финальная очистка выполнена: browser tab закрыт, viewport сброшен, API, Vite и изолированная PostgreSQL остановлены, тестовые базы и кластер удалены, `dotnet build-server shutdown` выполнен. Удалены распаковка DOCX, coverage, production `dist`, Vite cache, backend `bin`/`obj`, migration SQL и журналы задачи. Порты `5098`, `5183`, `55458` свободны; task-owned процессов и временных артефактов нет. Два `cua_node` сохранены как общие служебные процессы Codex; пользовательские процессы, базы и прежние артефакты не изменялись.
