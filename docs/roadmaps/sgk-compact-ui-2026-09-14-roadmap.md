...
- 2026-09-15 — В отчёте «Сборы» действия выгрузки перенесены в ту же внутреннюю grid-строку, что и поисковый контрол. Подпись «Все сборы», состояние поиска и карточки выбранных вариантов остаются отдельными нижними строками и больше не сдвигают XLSX/PDF вниз. Компонентная и CSS-регрессии защищают совместное размещение поиска и обеих кнопок; добавлена пользовательская запись «Что нового» версии `1.183.91`.
- 2026-09-15 — Browser-приёмка выполнена на отдельной PostgreSQL 17 с синтетическими showcase-данными строго при viewport `1366×768`, DPR 1. При пустом и выбранном фильтре верхняя координата поискового контрола, XLSX и PDF совпала (`174px`); документ сохранил `clientWidth = scrollWidth = 1366px`, обе кнопки находятся внутри viewport, ошибок и предупреждений console нет. Проверочная вкладка закрыта, временный viewport сброшен; снимок использовался только для визуальной проверки и не сохранялся.
- 2026-09-15 — Финальный gate выравнивания экспорта отчёта успешен. Frontend: focused `105/105`, полный suite `106/106` файлов и `1268/1268` тестов; statements `88,60%`, branches `81,55%`, functions `87,01%`, lines `89,77%`; lint, production build, npm audit и bundle `297286/297984` байт прошли. Backend: Release build без предупреждений, `2890/2890` тестов без пропусков на PostgreSQL 17 с ICU `ru-RU`, строки `91,30%`, ветви `76,45%`; format, privacy (`1318` файлов), аудит четырёх проектов, Docker distribution, отсутствие pending model changes и двукратное применение idempotent migration SQL подтверждены. Docker CLI на машине недоступен, поэтому Compose config/build оставлены обязательному CI. Первый backend-run был прерван новым пользовательским сообщением вместе с тестовой PostgreSQL; неполный coverage удалён, кластер восстановлен, а полный повтор завершился без ошибок.
- 2026-09-15 — Финальная очистка выравнивания экспорта выполнена: browser tab закрыт, viewport сброшен, API, Vite и PostgreSQL остановлены, `dotnet build-server shutdown` выполнен. Удалены одноразовый кластер и журнал, frontend/backend coverage, production `dist`, `node_modules`, backend `bin`/`obj` и migration SQL; task-owned процессов и временных артефактов не оставлено. Общие процессы Codex/CUA, language server и пользовательские сервисы не изменялись.
