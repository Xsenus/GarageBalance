...
- 2026-09-15 — В карточке «Сверка общего остатка» итоговые суммы перенесены из узкой правой колонки под пояснение и собраны в компактный вертикальный список. Подписи занимают доступную ширину, денежные значения выровнены справа табличными цифрами и защищены от сжатия; добавлена запись «Что нового» версии `1.183.93`.
- 2026-09-15 — Browser-приёмка карточки сверки выполнена на отдельной showcase-базе PostgreSQL 17 с реальным дефицитом `−10 789,14 руб.` строго при viewport `1366×768` и DPR `1`. Все три строки расположены ниже пояснения, денежные значения имеют одинаковую колонку `150px`, полностью видимы и не переполняются; карточка целиком находится в viewport (`top 529,72px`, `bottom 696,63px`), левая колонка не требует прокрутки (`683/683px`), document `1366/1366px`, ошибок и предупреждений console нет. Проверочная вкладка закрыта, временный viewport сброшен.
- 2026-09-15 — Финальный gate компоновки сверки фондов успешен. Frontend: focused `66/66`, полный suite `106/106` файлов и `1269/1269` тестов; statements `88,61%`, branches `81,58%`, functions `87,01%`, lines `89,79%`; lint, production build, npm audit и bundle `297208/297984` байт прошли. Backend: Release build без предупреждений, `2890/2890` тестов без пропусков на PostgreSQL 17 с ICU `ru-RU`, строки `91,30%`, ветви `76,46%`; format, privacy (`1318` файлов), аудит четырёх проектов, статическая Docker Compose/distribution-проверка, отсутствие pending model changes и двукратное применение idempotent migration SQL подтверждены. Docker CLI локально недоступен, контейнерная сборка оставлена обязательному GitHub Actions; push пока не выполнялся.
- 2026-09-15 — Финальная очистка компоновки сверки выполнена: проверочная browser-вкладка закрыта и viewport сброшен, API, Vite и изолированная PostgreSQL остановлены, `dotnet build-server shutdown` выполнен. Удалены временный кластер, frontend/backend coverage, production `dist`, зависимости frontend, `bin`, `obj` и migration SQL. Порты `5101`, `5182`, `5185` и `55464` свободны; task-owned процессов и одноразовых артефактов не осталось. Два процесса `cua_node` сохранены как общие служебные процессы Codex; пользовательские процессы и данные не изменялись.
