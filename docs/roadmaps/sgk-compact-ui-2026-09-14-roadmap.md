...
- 2026-09-15 — Исправлено положение признака обязательности у поля «Фонд» в форме «Добавить сбор»: звёздочка теперь формируется непосредственно у текста подписи перед клавиатурно-доступным значком справки и не переносится под него. Обычные подписи без справки сохранили прежнюю разметку. Компонентная и CSS-регрессии защищают структуру подписи и точный селектор обязательного маркера; добавлена пользовательская запись «Что нового» версии `1.183.87`.
- 2026-09-15 — Browser-приёмка выполнена на отдельной PostgreSQL 17.2 с ICU `ru-RU` и синтетическим администратором строго при viewport `1366×768`. Реальная форма показала `Фонд * ?` в одной строке: текст со звёздочкой, значок справки и контейнер подписи имеют одинаковые `y = 335,4375px` и высоту `15px`; псевдоэлемент содержит `" *"` и красный цвет `rgb(240, 68, 56)`. Document `1366/1366`, ошибок и предупреждений browser console нет; рабочие данные и пользовательские процессы не использовались.
- 2026-09-15 — Полный локальный gate исправления успешен. Frontend: focused `81/81`, полный suite `108/108` файлов и `1269/1269` тестов; statements `88,60%`, branches `81,55%`, functions `87,01%`, lines `89,77%`; lint, production build, npm audit и bundle `297229/297984` байта прошли. Backend: `2890/2890` тестов без пропусков на PostgreSQL, строки `91,28%`, ветви `76,45%`; format, privacy (`1318` файлов), аудит четырёх проектов, статическая проверка Docker-дистрибутива, отсутствие pending model changes и двукратное применение idempotent migration SQL (`426602` байта, 146 миграций) подтверждены. Docker CLI отсутствует, поэтому локальные Compose config/build недоступны и оставлены обязательному CI опубликованной ветки.
- 2026-09-15 — Финальная очистка исправления выполнена: временная browser-вкладка закрыта, viewport сброшен, API, Vite и изолированная PostgreSQL остановлены, `dotnet build-server shutdown` выполнен. Удалены тестовый кластер и база, coverage, production `dist`, восстановленный для проверок `node_modules`, backend `bin/obj`, migration SQL и одноразовые журналы. Общие процессы Codex и пользовательские сервисы не изменялись.
- 2026-09-15 — По замечанию пользователя кнопка «Назад» закреплена в общей верхней строке рабочих разделов: удалены обработчик, записывавший вертикальное смещение вложенной прокрутки, и связанная CSS-трансформация. Регрессионный сценарий проверяет прокрутку таблицы вниз и возврат наверх без изменения верхней строки; статическая проверка запрещает возвращать прежнюю переменную смещения. Добавлена запись «Что нового» версии `1.183.88`; номер оставлен отдельным от уже опубликованной ветки с версией `1.183.87`.
- 2026-09-15 — Browser-приёмка выполнена на отдельной showcase-базе PostgreSQL 17 с синтетическими данными строго при viewport `1366×768`. Тарифная область прокручена до нижней границы (`scrollTop = 154/154`), при этом кнопка возврата до и после прокрутки сохранила координаты `x=80`, `y=12`, размер `43×43` и видимость. Document `1366/1366`, ошибок и предупреждений console нет; проверочный снимок не сохранялся, поскольку пользователь его не запрашивал.
- 2026-09-15 — Финальный локальный gate исправления успешен. Frontend: focused `503/503`, полный suite `108/108` файлов и `1268/1268` тестов; statements `88,60%`, branches `81,55%`, functions `87,00%`, lines `89,76%`; lint, production build, npm audit и bundle `296971/297984` байт прошли. Backend: Release build без предупреждений, `2890/2890` тестов без пропусков на отдельной PostgreSQL 17 с ICU `ru-RU`, строки `91,28%`, ветви `76,45%`; format, privacy (`1318` файлов), аудит четырёх проектов, отсутствие pending model changes и двукратное применение idempotent migration SQL (`426602` байта, 146 миграций, 50 таблиц) подтверждены. Статическая Docker distribution проверка прошла; Docker CLI локально отсутствует, поэтому контейнерный gate остаётся за обязательным CI перед публикацией ветки.
