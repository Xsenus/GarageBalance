...
- 2026-09-15 — По замечанию пользователя исправлена верхняя строка формы поставщика: «Наименование», «Услуга» и «Фонд расходования» получили общую строку подписи и одинаковую высоту контролов `40px`; обязательная звёздочка фонда перенесена непосредственно после слов «Фонд расходования», перед значком справки. Добавлена CSS-регрессия и запись «Что нового» версии `1.183.90`.
- 2026-09-15 — Browser-приёмка формы нового поставщика выполнена на изолированной showcase-базе. Все три подписи имеют `top=104px`, все контролы — `top=124px`, `bottom=164px` и высоту `40px`; вычисленный псевдоэлемент звёздочки находится у текста «Фонд расходования», внешний псевдоэлемент отключён. При viewport `1280×720` document имеет `1280/1280px`, визуальных наложений и горизонтального overflow нет; проверочная вкладка закрыта без сохранения снимка.
- 2026-09-15 — Финальный локальный gate выравнивания полей успешен. Frontend: focused CSS `64/64`, старый нагрузочно-чувствительный сценарий меню пользователей отдельно `1/1`, стабильный полный повтор `106/106` файлов и `1268/1268` тестов; statements `88,60%`, branches `81,55%`, functions `87,01%`, lines `89,77%`; lint, production build, npm audit и bundle `297263/297984` байт прошли. Первый параллельный прогон при конкуренции с backend дал один сбой MutationObserver в старом сценарии, изолированный и полный последовательный повторы прошли без изменения таймаутов. Backend: Release build без предупреждений, `2890/2890` тестов без пропусков, строки `91,30%`, ветви `76,46%`; format, privacy (`1318` файлов), аудит четырёх проектов, статическая Docker distribution, отсутствие pending model changes и двукратное применение idempotent migration SQL (`426602` байта) подтверждены. Локальный Docker CLI недоступен; полный Docker build/start оставлен обязательному CI.
- 2026-09-15 — Финальная очистка выравнивания полей выполнена: проверочная browser-вкладка закрыта, API, Vite и изолированная PostgreSQL остановлены, `dotnet build-server shutdown` выполнен. Удалены временный кластер и тестовая база, frontend/backend coverage, `dist`, `node_modules`, `bin`, `obj` и migration SQL. Порты `5098`, `5183`, `55461` свободны; task-owned процессов и временных артефактов не осталось, пользовательские и общие служебные процессы не изменялись.
