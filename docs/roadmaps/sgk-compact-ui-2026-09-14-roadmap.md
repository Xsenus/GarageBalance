...
- 2026-09-15 — В настройках исправлены отступы общей кнопки возврата: компактный desktop-режим теперь использует те же смещения `25px` сверху и `27px` слева, что отчёты и платежи. CSS-регрессия защищает отдельное правило раздела от возврата кнопки к краям рабочей области; добавлена запись «Что нового» версии `1.183.92`.
- 2026-09-15 — Browser-приёмка настроек выполнена на изолированных синтетических данных строго при viewport `1366×768`: кнопка расположена на `27px` правее и `25px` ниже начала workspace, сохраняет аккуратный внутренний отступ навигационной панели (`15px/13px`) и расстояние `13px` до вкладок; document `1366/1366`, ошибок и предупреждений console нет. Вкладка закрыта и временный viewport сброшен.
- 2026-09-15 — Финальный gate отступов кнопки в настройках успешен. Frontend: focused `79/79`, полный suite `106/106` файлов и `1268/1268` тестов; statements `88,60%`, branches `81,55%`, functions `87,01%`, lines `89,77%`; lint, production build, npm audit и bundle `297192/297984` байта прошли. Backend: Release build без предупреждений, `2890/2890` тестов без пропусков на PostgreSQL 17 с ICU `ru-RU`, строки `91,30%`, ветви `76,46%`; format, privacy (`1318` файлов), аудит четырёх проектов, отсутствие pending model changes и двукратное применение idempotent migration SQL подтверждены. Статические Docker distribution-проверки прошли; Docker CLI локально недоступен, поэтому Compose build оставлен обязательному CI.
- 2026-09-15 — Финальная очистка отступов кнопки выполнена: изолированная PostgreSQL остановлена, `dotnet build-server shutdown` выполнен, временный кластер, frontend/backend coverage, `node_modules`, production `dist`, `bin`, `obj` и migration SQL удалены. Порты `5100`, `5185` и `55463` свободны; task-owned процессов и временных артефактов не осталось, пользовательские и общие служебные процессы не изменялись.
