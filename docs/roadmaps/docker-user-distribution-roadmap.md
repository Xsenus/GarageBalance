...
- [x] На чистой базе первый запуск создаёт первоначальные тарифы, регулярные услуги, нерегулярные платежи и готового администратора с уникальным локальным паролем.
...
- 2026-09-01: итоговая интеграционная проверка основной ветки на чистом Compose-томе выявила отсутствующий `curl` в API-образе при настроенном через него healthcheck. Runtime-образ дополнен зафиксированной версией `curl`, контракт Dockerfile обновлён; повторная сборка подтвердила healthy-статус PostgreSQL, API и frontend, применение всех 125 миграций, прямой и проксированный readiness `ok`, отсутствие критических ошибок в контейнерных логах и чистую браузерную консоль на странице авторизации.
- 2026-09-04: сценарий чистой установки дополнен автоматическим созданием администратора `admin@garagebalance.local`. Установщик генерирует отдельный случайный пароль, сохраняет его только в локальном `admin-credentials.txt`, не выводит в консоль/diagnostics и после успешного health-check отключает повторную инициализацию, удаляет пароль из `.env` и пересоздаёт API-контейнер без исходного пароля в окружении. PostgreSQL-регрессия закрепляет наличие первоначальных тарифов, шести регулярных услуг и трёх нерегулярных платежей, создание администратора, его роль, права, вход и аудит. Docker release gate разделён на параллельные backend, frontend и distribution jobs, после которых разрешена сборка образов. Проверено на PostgreSQL 17: полный backend `2501/2501`, покрытие `90,82%` строк / `74,59%` ветвей; frontend `1038/1038`, `87,64%` statements / `79,90%` branches / `85,34%` functions / `88,83%` lines, lint, production build и bundle `277,8/278,0` КиБ. Успешны format, privacy (`1696` файлов), NuGet/npm audit, actionlint, PowerShell, release JSON, Docker-контракты и двукратное применение идемпотентного SQL (`383225` байт, `46` таблиц, `131` миграция, `9` тарифов, `6` услуг, `3` нерегулярных платежа). Реальный Production-запуск API подтвердил health `ok`, вход администратора, роль и права. Фактическая пересборка контейнеров на этой рабочей машине недоступна из-за отсутствия Docker Engine; финальная acceptance чистого Windows-компьютера остаётся открытой.
