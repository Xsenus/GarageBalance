...
- [ ] Параметр PostgreSQL `garagebalance.demo_seed_enabled` отсутствует или выключен; в чистой рабочей базе нет гаражей и показаний прежнего демонстрационного набора за 2021–2026 годы.
...
- [ ] После очистки прежнего demo-seed сохранены импортированные, клиентские и вручную добавленные показания, включая записи с датами, совпадающими с демонстрационным периодом.

## 5. Фиксация результата

- [ ] В журнале технических работ записаны: имена временных баз, путь к backup, путь к SQL-скрипту, результат clean DB, результат post-import DB, результат повторного применения и health/smoke.
...
- [ ] Результат зафиксирован в журнале технических работ без персональных, финансовых и импортных данных.
