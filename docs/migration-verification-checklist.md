...
- [ ] В `docs/project-roadmap.md` в `История выполнения` записаны: имена временных баз, путь к backup, путь к SQL-скрипту, результат clean DB, результат post-import DB, результат повторного применения и health/smoke.
