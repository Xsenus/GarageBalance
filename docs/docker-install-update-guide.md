...
Backend-образ собирается из закреплённых по digest образов .NET, с зафиксированным NuGet-графом и точными версиями PostgreSQL client 17. Публичный ключ пакетного репозитория входит в исходники и перед установкой проверяется по SHA-256, поэтому сборка не зависит от отдельного TLS-запроса за ключом. Если версия PostgreSQL-клиента или базового .NET-образа обновляется, одновременно обновите lock-файл, digest/checksum и пройдите реальную сборку образа с проверкой `pg_dump --version` и `pg_restore --version`.
