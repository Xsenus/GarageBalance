...
- генерирует уникальный пароль и создаёт первоначального администратора с логином `admin@garagebalance.local`;
...
После создания администратора установщик автоматически пересоздаёт API-контейнер без исходного пароля в его окружении. Затем откройте `admin-credentials.txt` в папке GarageBalance и войдите указанными логином и паролем. Пароль не выводится в консоль и не попадает в диагностику. Сохраните его в защищённом менеджере паролей и удалите `admin-credentials.txt` после первого успешного входа. Общего пароля для всех установок нет: каждое чистое развёртывание получает новый случайный пароль.

На чистой базе миграции сразу создают рабочий первоначальный каталог: воду, электроэнергию, членский и целевой взносы, вывоз мусора, наружное освещение и три нерегулярных платежа — вступительный взнос, подключение канализации и подключение к линии электросети. Перед вводом реальных начислений администратор обязан проверить ставки, сроки и даты действия в разделе `Тарифы и сборы`: предзаполнение ускоряет настройку, но не заменяет решение кооператива об актуальных суммах.
...
- одноразовые данные первого входа — в `admin-credentials.txt`, который следует удалить после безопасного сохранения пароля.
...
Во время `up -d` Docker заменяет контейнеры, но подключает те же volumes и backup-папку. Поэтому записи PostgreSQL не стираются. Перед миграцией базы, в которой уже есть таблицы, API дополнительно создает `pre_update` backup; если `pg_dump` или проверка файла завершается ошибкой, миграция и запуск API останавливаются вместо рискованного продолжения. На первой установке с пустой базой миграции применяются без такой копии.
