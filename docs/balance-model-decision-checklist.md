...
- [x] Утверждено 17.07.2026: платеж закрывает старейшие непогашенные начисления того же гаража и вида поступления; нераспределенный остаток автоматически остается переплатой и уменьшает будущие обязательства этой пары.
...
- [x] Утвержденное FIFO-распределение, частичная оплата и нераспределенная переплата покрыты backend unit- и PostgreSQL integration-тестами.
- [x] Отчеты и расшифровка долга используют активные распределения и показывают переплату отдельно от просрочки.
