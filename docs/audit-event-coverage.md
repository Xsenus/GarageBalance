...
Дата инвентаризации: 19.07.2026.
...
- Свободный текст summary, причин и комментариев маскирует email, телефоны, паспортные данные, адреса с явной меткой, токены, секреты и длинные номера счетов до записи события.
...
| `finance.accrual_updated_from_meter_reading` | `accrual` | Да | Нет | Да | `FinanceServiceTests`, `PostgreSqlMeterReadingConcurrencyIntegrationTests` | атомарный пересчет неоплаченного начисления по подтвержденному расходу; частичная или полная оплата блокирует изменение |
