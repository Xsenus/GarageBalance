...
No production behavior, schema or data-cleanup policy changed in this audit, so an end-user «Что нового» entry is not required. Local PostgreSQL credentials remain unavailable on this workstation; database-specific verification used PostgreSQL integration tests and the authorized VPS read-only checks. GitHub Actions and post-deployment VPS results are recorded after publication.
