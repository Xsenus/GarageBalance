...
- [x] Final repeated verification passed: 1504 backend tests and 419 frontend tests, ESLint, TypeScript, production build, bundle budget (`74.1 KiB` main JS, `17.0 KiB` CSS, `212.4 KiB` total gzip), backend format, Docker Compose configuration, release JSON, strict UTF-8/no BOM, `git diff --check` and idempotent EF migration SQL. VPS service/nginx/public health, warning journal, backup catalog and the retained-nine-tariffs/zero-operational-rows database state were checked again.
