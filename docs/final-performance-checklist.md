...
Рабочие таблицы используют серверную пагинацию, поэтому дополнительная DOM-виртуализация
для них не требуется. Отчёты составляют отдельное исключение по согласованному
пользовательскому сценарию: frontend последовательно получает ограниченные серверные
пакеты и показывает весь отчёт вниз без пользовательской пагинации. Это не превращает
backend-запрос в неограниченную выборку и не влияет на остальные рабочие таблицы.
...
текущего пользователя и все 12 рабочих разделов: пользователей, тарифы, контрагентов,
справочники, показания, платежи, фонды, отчёты, импорт, аудит, «Что нового» и настройки.
Каждый рабочий сценарий содержит стабильную метку `workspaceSection`, поэтому тест
не позволит незаметно потерять раздел из эксплуатационной проверки. Для одиночной проверки используется `-ScenarioName health`; для CI —
...
## Исходный baseline первого открытия

Контрольный снимок хранится в
`infrastructure/performance/frontend-first-open-baseline.json`. Он снят в отдельной
БД на обезличенном профиле `500` гаражей × `60` месяцев через same-origin proxy с
`Cache-Control: no-store`. Proxy считал только метод, путь без query string, статус,
время и полную длину ответа; bearer token, JSON-тела и пользовательские значения не
сохранялись.

| Раздел | Всего запросов | API | Статика | JSON, байт | Статика, байт |
| --- | ---: | ---: | ---: | ---: | ---: |
| Пользователи | 12 | 2 | 10 | 1 261 | 31 642 |
| Тарифы и сборы | 17 | 9 | 8 | 24 507 | 124 729 |
| Контрагенты | 7 | 2 | 5 | 169 665 | 109 406 |
| Справочники | 3 | 2 | 1 | 220 905 | 50 100 |
| Показания | 2 | 1 | 1 | 1 741 | 11 554 |
| Платежи | 4 | 2 | 2 | 42 537 | 188 698 |
| Фонды | 5 | 2 | 3 | 1 490 | 36 963 |
| Отчёты | 3 | 1 | 2 | 781 | 49 624 |
| Импорт | 4 | 3 | 1 | 528 | 32 652 |
| История изменений | 2 | 1 | 1 | 16 771 | 21 170 |
| Что нового | 2 | 1 | 1 | 8 577 | 8 661 |
| Настройки | 1 | 0 | 1 | 0 | 48 163 |

Холодная оболочка без feature-раздела занимает три ресурса: `420 029` байт без
сжатия и `105 316` байт gzip. Наиболее заметные исходные цели оптимизации —
полные reference-ответы гаражей/владельцев в «Справочниках» и «Контрагентах»,
девять API-запросов «Тарифов и сборов», а также feature chunk «Платежей».
После соответствующей вехи снимок повторяется на том же seed и сравнивается с этим
файлом; уменьшение размера не должно достигаться удалением данных текущей страницы.

## Итоговый baseline после оптимизации

Итоговый privacy-safe снимок хранится в
`infrastructure/performance/frontend-final-baseline.json`. API каждого раздела
повторён по пять раз на том же профиле `500 × 60`; в файл попали только агрегаты,
пути без query string не сохранялись, тела ответов и токен отсутствуют.

Все двенадцать разделов завершили корректные GET-сценарии с `0%` ошибок. Худший
локальный p95 составил `48,1` мс у контрагентов, у отчёта — `44,8` мс. Первое
открытие «Тарифов и сборов» уменьшено с девяти до семи API-запросов: устаревшие
загрузка и сохранение полного снимка формы удалены.

Web Vitals измеряются для документа, а не отдельно для SPA-вкладок. Поэтому
публичная холодная оболочка зафиксирована единым замером: HTTP/2, четыре запроса
вместе с favicon, `108 815` переданных байт, LCP `696` мс, INP `32` мс, CLS `0`,
ошибок консоли `0`. Для каждого раздела отдельно сохранены количество запросов,
сжатые размеры API/статики, p50/p95/max, error rate и один React commit на
осознанный переход. Быстрая серия из двенадцати переходов объединяется в один
финальный commit; сворачивание боковой панели рабочую область не перерисовывает.

Публичный маршрут направлен прямо на nginx в режиме DNS-only. Текущая сборка nginx
поддерживает HTTP/2 и gzip, но не содержит модулей Brotli и QUIC/HTTP/3. Это
зафиксированная характеристика прямого origin, а не скрытая ошибка приложения:
основной JS с gzip успешно загружен `30/30` раз.
