...
Независимый каталог услуг поставщиков хранится в `supplier_services` и обслуживается `SupplierServiceCatalog`. Маршрут `/api/dictionaries/supplier-services` поддерживает GET с `search`, `offset`, `limit`, `includeArchived`, POST с `name` и PUT `/{id}` с `name` и `version`. Чтение требует `dictionaries.read`, запись — `dictionaries.write`; отсутствующая версия даёт 400, устаревшая — 409, дублирующее активное имя — 409. Сервис ограничивает страницу 500 строками, сохраняет запись вместе с аудитом и сериализует изменения существующей блокировкой словарно-финансовых операций. Тарифных полей у этого API нет. SupplierDto возвращает supplierServiceId/supplierServiceName, UpsertSupplierRequest принимает supplierServiceId. Новая связь используется в поиске и сортировке поставщиков, начислениях, выплатах и ведомости расходов. Старая ChargeServiceSettingId остаётся в базе для истории миграции; изменения карточки не записывают эту колонку. Backend и frontend этого изменения выпускаются вместе.

PostgreSQL выполняет поиск каталога и пагинацию на сервере. Только в тестовом SQLite поиск использует отдельный fallback с `OrdinalIgnoreCase`, поскольку встроенный `lower()` SQLite не приводит кириллицу к нижнему регистру. Этот путь не предназначен для рабочей базы. Клиентский кэш каталога инвалидирует имена поставщиков после успешной записи, но не кэш тарифов владельцев.
...
Frontend разделять на feature-модули: `frontend/src/features/workspace/AppShell.tsx`, `frontend/src/features/workspace/Workspace.tsx`, `frontend/src/features/finance/FinancePanel.tsx`, `frontend/src/features/contractors/ContractorsPanel.tsx`, `frontend/src/features/tariffs/TariffsAndFeesPanel.tsx`, `frontend/src/features/dictionaries/DictionaryPanel.tsx`, `frontend/src/features/users/UserManagementPanel.tsx`, `frontend/src/features/reports/ReportPanel.tsx`, `frontend/src/features/audit/AuditPanel.tsx`, `frontend/src/shared/workspaceNavigation.ts`, `frontend/src/features/meterReadings/MeterReadingsPanel.tsx`, `frontend/src/shared/prototypeEditing.ts`, `frontend/src/features/import/ImportPanel.tsx`, `frontend/src/features/funds/FundsPanel.tsx`, `frontend/src/features/settings/PasswordPanel.tsx`, `frontend/src/features/auth/AuthGate.tsx`, `frontend/src/features/releases/ReleasePanel.tsx`; повторяемые контролы оставлять в shared UI.
...
Публичными остаются `GET /health/live`, `GET /health/ready` (и совместимый alias `GET /health`), первичное создание администратора и вход. Liveness проверяет только процесс, readiness — доступность PostgreSQL. Рабочие маршруты требуют JWT и соответствующего permission.
