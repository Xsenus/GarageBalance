...
- [x] Годовая таблица показаний использует отдельную компактную выборку: страница активных гаражей ограничивается до materialization, а показания выбираются только для этих гаражей, типа счетчика и выбранного года.
...
- 2026-07-13 — устранена медленная загрузка годовой таблицы показаний: универсальные запросы гаражей с расчетом балансов и расширенных показаний заменены одной компактной постраничной проекцией. Добавлен частичный индекс PostgreSQL `(MeterKind, AccountingMonth, GarageId)` для активных показаний. Проверка закреплена service/controller/UI-тестами и `BackendPerformanceGuardTests`; живой замер выполнен на локальной PostgreSQL после применения миграции.

## Release Notes

- [x] Новая запись "Что нового" не нужна для прежнего test/evidence-среза, не менявшего production-поведение; для нынешнего ускорения годовой таблицы добавлен отдельный выпуск `2026-07-13-meter-readings-fast-year-page`, потому что изменились видимая загрузка и навигация формы.
