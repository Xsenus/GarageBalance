...
- `meter_readings` - показания воды и электричества; электрическая энергия хранится и показывается в `кВт·ч`. Уникальность: `GarageId + MeterKind + AccountingMonth`; `HasGapWarning` фиксирует разрыв истории.
