...
Сформированные системой комментарии в `financial_operations.Comment`, `supplier_accruals.Comment`, `meter_readings.Comment` и причины в `fund_operations.Reason` хранятся как PostgreSQL `text`. Лимит вводимого пользователем текста в API остаётся 1000 символов; добавление системного пояснения или причины отмены не должно обрезать исходный текст или срывать сохранение. Миграция `PreserveGeneratedFinancialComments` расширяет эти поля без потери данных. Обратное сужение до 1000 символов при наличии более длинных значений завершается ошибкой, поэтому для отката такой установки используют проверенную резервную копию, а не обрезку финансовой истории.
...
- `opening_balance_adjustments` - неизменяемые документы корректировки начальных данных гаража или поставщика. Для гаража документ сохраняет прежний и новый общий начальный баланс, а также прежнюю и новую просроченную часть; для поставщика просроченная часть не применяется. Каждая запись содержит бизнес-дату, причину, автора и время создания, а изменение дублируется в журнале аудита.
