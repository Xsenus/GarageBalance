...
- `charge_service_settings` - настройки услуг раздела "Тарифы и сборы": регулярность, периодичность, месяц начала учета, день/месяц оплаты, перенос долга, единица измерения, признаки счетчика и пороговой тарификации, а также ссылки на `IncomeTypeId` и `TariffId` для генерации начислений. Активное имя уникально через filtered unique index по `Name` при `IsArchived = false`; индексы покрывают `IsRegular`, `IsMetered`, `HasTieredTariff`, `IncomeTypeId` и `TariffId`.
