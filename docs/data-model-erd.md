...
- `staff_employment_periods` - интервалы месяцев, в которых сотруднику начисляется оклад. Начало и необязательное окончание нормализованы на первый день месяца, окончание не может предшествовать началу, а частичный уникальный индекс допускает только один открытый период сотрудника. Архивирование закрывает период, восстановление открывает новый и не начисляет долг за месяцы перерыва.
