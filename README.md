...
Архивирование записей справочников и тарифов подтверждается отдельным dialog: первый клик только открывает проверку действия, а API вызывается после явного нажатия "Архивировать запись". Это защищает владельцев, гаражи, поставщиков, виды операций и тарифы от случайного архивирования, сохраняя soft-archive и audit-историю.
...
Списки справочников показывают первые 5 записей и выводят счетчик вида `Показано 5 из N записей`, если в разделе есть дополнительные элементы. Так оператор видит, что список обрезан для компактности, а не считает данные потерянными. Кнопка `Показать все записи` раскрывает полный список внутри текущего справочника, а `Свернуть список` возвращает компактный режим.
