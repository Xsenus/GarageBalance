...
Сервисные ошибки API возвращаются в едином формате `ProblemDetails`: `title` содержит код ошибки, `detail` - человекочитаемое описание, `status` - HTTP-статус, а `extensions.code` дублирует машинный код для frontend-обработки. Контракт применяется к auth, пользователям, справочникам, финансам, отчетам, импорту и "Что нового".
