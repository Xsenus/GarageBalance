...
Поступления и выплаты отменяются без физического удаления: пользователь указывает причину, backend ставит `IsCanceled`, добавляет `Отменено: ...` в комментарий и пишет audit-событие `finance.operation_canceled`. Активные списки, сводка и расчеты задолженности исключают отмененные операции.
