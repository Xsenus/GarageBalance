...
Скрипт проверит Docker, сам создаст локальный `.env` с уникальными секретами, загрузит из ZIP готовые образы API, frontend и PostgreSQL, применит миграции и дождётся успешного `/health`. При первом открытии создайте администратора.
