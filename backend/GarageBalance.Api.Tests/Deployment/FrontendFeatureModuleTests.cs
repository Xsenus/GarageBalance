...
public sealed class FrontendFeatureModuleTests
{
    ...
    public void AppShellRemainsInItsFeatureModule()
    {
        ...
        Assert.Contains("frontend/src/features/workspace/AppShell.tsx", File.ReadAllText(Path.Combine(repositoryRoot, "docs", "development-guide.md")), StringComparison.Ordinal);
    }
    ...
    public void WorkspaceRemainsInItsFeatureModule()
    {
        ...
        Assert.Contains("frontend/src/features/workspace/Workspace.tsx", File.ReadAllText(Path.Combine(repositoryRoot, "docs", "development-guide.md")), StringComparison.Ordinal);
    }
    ...
    public void ContractorsPanelRemainsInItsFeatureModule()
    {
        ...
        Assert.Contains("frontend/src/features/contractors/ContractorsPanel.tsx", File.ReadAllText(Path.Combine(repositoryRoot, "docs", "development-guide.md")), StringComparison.Ordinal);
    }
    ...
    {
        var directLazyImport = $"const {componentName} = lazy(() => import('{modulePath}')";
        if (source.Contains(directLazyImport, StringComparison.Ordinal))
        {
            return;
        }
        ...
        var loaderSource = File.ReadAllText(Path.Combine(
            ...
            "features",
            "workspace",
            "workspaceSectionLoader.ts"));

        Assert.Contains($"export const {componentName} = lazy(", loaderSource, StringComparison.Ordinal);
        Assert.Contains($"import('{modulePath}')", loaderSource, StringComparison.Ordinal);
        Assert.Contains("createRetryableLazyLoader", loaderSource, StringComparison.Ordinal);
    }
