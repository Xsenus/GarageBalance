...
public sealed class DictionariesControllerTests
{
    ...
    public async Task ListEndpoints_PassLimitToService()
    {
        ...
        await controller.GetTariffsPage("meter", 20, 15, true, CancellationToken.None);
        ...
        Assert.Equal(("meter", 20, 15, true), service.LastTariffPageRequest);
    ...
    [Fact]
    public async Task CreateSupplier_ReturnsCreatedAndPassesActorUserId()
    {
        var actorUserId = Guid.NewGuid();
        var groupId = Guid.NewGuid();
        ...
        var service = new FakeDictionaryService
        {
            CreateSupplierResult = DictionaryResult<SupplierDto>.Success(new SupplierDto(supplierId, "Водоканал", groupId, "Коммунальные услуги", "5401000000", "Юр. адрес", "Петров", "+7", "mail@example.com", 32000m, "Основной", false))
        };
        var controller = CreateController(service, actorUserId);

        var result = await controller.CreateSupplier(
            new UpsertSupplierRequest("Водоканал", groupId, "5401000000", "Юр. адрес", "Петров", "+7", "mail@example.com", 32000m, "Основной"),
            CancellationToken.None);

        var created = Assert.IsType<CreatedAtActionResult>(result.Result);
        var dto = Assert.IsType<SupplierDto>(created.Value);
        Assert.Equal(supplierId, dto.Id);
        Assert.Equal(nameof(DictionariesController.GetSuppliers), created.ActionName);
        Assert.Equal(actorUserId, service.LastActorUserId);
    }
    ...
    public async Task CreateSupplier_ReturnsNotFoundForMissingGroup()
    {
        ...
        var result = await controller.CreateSupplier(
            new UpsertSupplierRequest("Водоканал", groupId, null, null, null, null, null, 0, null),
            CancellationToken.None);
    ...
    public async Task UpdateSupplier_ReturnsOkAndPassesActorUserId()
    {
        ...
        var service = new FakeDictionaryService
        {
            UpdateSupplierResult = DictionaryResult<SupplierDto>.Success(new SupplierDto(supplierId, "Водоканал", groupId, "Коммунальные услуги", "5401000000", "Новый адрес", "Петров", "+7", "mail@example.com", 32000m, "Обновлен", false))
        };
        var controller = CreateController(service, actorUserId);

        var result = await controller.UpdateSupplier(
            supplierId,
            new UpsertSupplierRequest("Водоканал", groupId, "5401000000", "Новый адрес", "Петров", "+7", "mail@example.com", 32000m, "Обновлен"),
            CancellationToken.None);

        var ok = Assert.IsType<OkObjectResult>(result.Result);
        var dto = Assert.IsType<SupplierDto>(ok.Value);
        Assert.Equal(supplierId, dto.Id);
        Assert.Equal("Новый адрес", dto.LegalAddress);
        Assert.Equal(actorUserId, service.LastActorUserId);
        Assert.Equal(supplierId, service.LastSupplierId);
    }

    [Fact]
    public async Task ArchiveSupplier_ReturnsNoContentAndPassesActorUserId()
    {
        var actorUserId = Guid.NewGuid();
        var groupId = Guid.NewGuid();
        var supplierId = Guid.NewGuid();
        var service = new FakeDictionaryService
        {
            ArchiveSupplierResult = DictionaryResult<SupplierDto>.Success(new SupplierDto(supplierId, "Водоканал", groupId, "Коммунальные услуги", null, null, null, null, null, 0, null, true))
        };
        var controller = CreateController(service, actorUserId);

        var result = await controller.ArchiveSupplier(supplierId, new ArchiveDictionaryEntryRequest("Поставщик больше не используется"), CancellationToken.None);

        Assert.IsType<NoContentResult>(result);
        Assert.Equal(actorUserId, service.LastActorUserId);
        Assert.Equal(supplierId, service.LastSupplierId);
        Assert.Equal("Поставщик больше не используется", service.LastArchiveReason);
    }
    ...
    public async Task CreateChargeServiceWithTariff_ReturnsCreatedCompositeAndPassesActorUserId()
    {
        ...
        var createdService = new ChargeServiceSettingDto(serviceId, "Охрана", true, 1, 1, 30, null, 30, Guid.NewGuid(), tariffId, false, false, "руб.", false);
        var createdTariff = new TariffDto(tariffId, "Охрана — тариф", "fixed", 1750m, new DateOnly(2026, 7, 23), null, false);
        var service = new FakeDictionaryService
        {
            CreateChargeServiceWithTariffResult = DictionaryResult<CreatedChargeServiceWithTariffDto>.Success(
                new CreatedChargeServiceWithTariffDto(createdService, createdTariff))
        };
        var controller = CreateController(service, actorUserId);
        var request = new CreateChargeServiceWithTariffRequest(
            new UpsertChargeServiceSettingRequest("Охрана", true, 1, 1, 30, null, 30, false, false, "руб.", createdService.IncomeTypeId, Guid.NewGuid()),
            1750m,
            new DateOnly(2026, 7, 23));

        var result = await controller.CreateChargeServiceWithTariff(request, CancellationToken.None);

        var created = Assert.IsType<CreatedAtActionResult>(result.Result);
        var dto = Assert.IsType<CreatedChargeServiceWithTariffDto>(created.Value);
        Assert.Equal(serviceId, dto.Service.Id);
        Assert.Equal(1750m, dto.Tariff.Rate);
    ...
    public async Task CreateChargeServiceWithTariff_ReturnsConflictWithoutPartialResponse()
    {
        var controller = CreateController(new FakeDictionaryService
        {
            CreateChargeServiceWithTariffResult = DictionaryResult<CreatedChargeServiceWithTariffDto>.Failure(
                "charge_service_duplicate",
                "Услуга с таким наименованием уже существует.")
        });
        var request = new CreateChargeServiceWithTariffRequest(
            new UpsertChargeServiceSettingRequest("Охрана", true, 1, 1, 30, null, 30, false, false, "руб.", Guid.NewGuid(), Guid.NewGuid()),
            1750m,
            new DateOnly(2026, 7, 23));
    ...
    public async Task SupplierContactsAndStaffEndpoints_PassFiltersAndActorToService()
    {
        ...
        Assert.Equal((supplierId, "петр", 20, 10, "status", "desc", true), service.LastSupplierContactPageRequest);
    ...
    [Fact]
    public async Task UpdateStaffDepartment_ReturnsOkAndPassesActorUserId()
    {
        var actorUserId = Guid.NewGuid();
        var departmentId = Guid.NewGuid();
        var service = new FakeDictionaryService
        {
            UpdateStaffDepartmentResult = DictionaryResult<StaffDepartmentDto>.Success(new StaffDepartmentDto(departmentId, "Руководство", false))
        };
        var controller = CreateController(service, actorUserId);

        var result = await controller.UpdateStaffDepartment(departmentId, new UpsertStaffDepartmentRequest("Руководство"), CancellationToken.None);

        var ok = Assert.IsType<OkObjectResult>(result.Result);
        var dto = Assert.IsType<StaffDepartmentDto>(ok.Value);
        Assert.Equal(departmentId, dto.Id);
        Assert.Equal("Руководство", dto.Name);
        Assert.Equal(actorUserId, service.LastActorUserId);
        Assert.Equal(departmentId, service.LastStaffDepartmentId);
    }

    [Fact]
    public async Task ArchiveStaffDepartment_ReturnsNoContentAndPassesActorUserId()
    {
        var actorUserId = Guid.NewGuid();
        var departmentId = Guid.NewGuid();
        var service = new FakeDictionaryService
        {
            ArchiveStaffDepartmentResult = DictionaryResult<StaffDepartmentDto>.Success(new StaffDepartmentDto(departmentId, "Бухгалтерия", true))
        };
        var controller = CreateController(service, actorUserId);

        var result = await controller.ArchiveStaffDepartment(departmentId, new ArchiveDictionaryEntryRequest("Отдел больше не используется"), CancellationToken.None);

        Assert.IsType<NoContentResult>(result);
        Assert.Equal(actorUserId, service.LastActorUserId);
        Assert.Equal(departmentId, service.LastStaffDepartmentId);
        Assert.Equal("Отдел больше не используется", service.LastArchiveReason);
    }

    [Fact]
    public async Task RestoreStaffDepartment_ReturnsOkActiveRecordAndPassesActorUserId()
    {
        var actorUserId = Guid.NewGuid();
        var departmentId = Guid.NewGuid();
        var service = new FakeDictionaryService
        {
            RestoreStaffDepartmentResult = DictionaryResult<StaffDepartmentDto>.Success(new StaffDepartmentDto(departmentId, "Бухгалтерия", false))
        };
        var controller = CreateController(service, actorUserId);

        var result = await controller.RestoreStaffDepartment(departmentId, CancellationToken.None);
        ...
        Assert.False(dto.IsArchived);
        Assert.Equal(actorUserId, service.LastActorUserId);
        Assert.Equal(departmentId, service.LastRestoreId);
    }
    ...
    private sealed class FakeDictionaryService : IDictionaryService
    {
        ...
        public DictionaryResult<StaffDepartmentDto> UpdateStaffDepartmentResult { get; init; } = DictionaryResult<StaffDepartmentDto>.Failure("not_configured", "Not configured.");
        public DictionaryResult<StaffDepartmentDto> ArchiveStaffDepartmentResult { get; init; } = DictionaryResult<StaffDepartmentDto>.Failure("not_configured", "Not configured.");
        ...
        {
            LastActorUserId = actorUserId;
            return Task.FromResult(CreateSupplierResult);
        }
        ...
        public Task<DictionaryResult<SupplierDto>> ArchiveSupplierAsync(Guid id, string reason, Guid? actorUserId, CancellationToken cancellationToken)
        {
            ...
            LastSupplierId = id;
            LastArchiveReason = reason;
            return Task.FromResult(ArchiveSupplierResult);
        }
        ...
        public Task<PagedResult<SupplierContactDto>> GetSupplierContactsPageAsync(Guid? supplierId, string? search, int? offset, int? limit, string? sortBy, string? sortDirection, CancellationToken cancellationToken, bool includeArchived = false)
        {
            LastSupplierContactPageRequest = (supplierId, search, offset, limit, sortBy, sortDirection, includeArchived);
            return Task.FromResult(new PagedResult<SupplierContactDto>([], 0, offset ?? 0, limit ?? 100));
        }
        ...
        {
            LastActorUserId = actorUserId;
            LastStaffDepartmentId = id;
            return Task.FromResult(UpdateStaffDepartmentResult);
        }
        ...
        public Task<DictionaryResult<StaffDepartmentDto>> ArchiveStaffDepartmentAsync(Guid id, string reason, Guid? actorUserId, CancellationToken cancellationToken)
        {
            ...
            LastStaffDepartmentId = id;
            LastArchiveReason = reason;
            return Task.FromResult(ArchiveStaffDepartmentResult);
        }
        ...
        public Task<PagedResult<StaffMemberDto>> GetStaffMembersPageAsync(Guid? departmentId, string? search, int? offset, int? limit, string? sortBy, string? sortDirection, CancellationToken cancellationToken, bool includeArchived = false)
        {
            LastStaffMemberPageRequest = (departmentId, search, offset, limit, sortBy, sortDirection, includeArchived);
        ...
        {
            LastTariffPageRequest = (search, offset, limit, includeArchived);
        ...
        public Task<DictionaryResult<CreatedChargeServiceWithTariffDto>> CreateChargeServiceWithTariffAsync(CreateChargeServiceWithTariffRequest request, Guid? actorUserId, CancellationToken cancellationToken)
        {
            LastActorUserId = actorUserId;
            return Task.FromResult(CreateChargeServiceWithTariffResult);
        }
