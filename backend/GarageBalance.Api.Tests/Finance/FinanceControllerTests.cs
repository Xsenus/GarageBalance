...
public sealed class FinanceControllerTests
{
    ...
    [Fact]
    public async Task GarageAnnualPaymentsEndpoints_PassGarageYearAndActorToService()
    {
        var garageId = Guid.NewGuid();
        var actorUserId = Guid.NewGuid();
        var annualPayments = new GarageAnnualPaymentsDto(
            garageId,
            "12",
            "Иванов Иван",
            2026,
        ...
        var service = new FakeFinanceService
        {
            GarageAnnualPaymentsResult = FinanceResult<GarageAnnualPaymentsDto>.Success(annualPayments)
        };
        var controller = CreateController(service, actorUserId);

        var read = await controller.GetGarageAnnualPayments(garageId, 2026, CancellationToken.None);
        var calculated = await controller.CalculateGarageAnnualPayments(garageId, 2026, CancellationToken.None);

        Assert.Same(annualPayments, Assert.IsType<OkObjectResult>(read.Result).Value);
        Assert.Same(annualPayments, Assert.IsType<OkObjectResult>(calculated.Result).Value);
        Assert.Equal(garageId, service.LastGarageAnnualPaymentsGarageId);
        Assert.Equal(2026, service.LastGarageAnnualPaymentsYear);
        Assert.Equal(actorUserId, service.LastGarageAnnualPaymentsActorUserId);
    }

    [Fact]
    public async Task PreviewGarageAnnualPayments_PassesGarageParametersToService()
    {
        var preview = new GarageAnnualPaymentPreviewDto(2026, []);
        var service = new FakeFinanceService
        {
            GarageAnnualPaymentPreviewResult = FinanceResult<GarageAnnualPaymentPreviewDto>.Success(preview)
        };
        var request = new GarageAnnualPaymentPreviewRequest(3, 2, 10m, 20m);

        var result = await CreateController(service).PreviewGarageAnnualPayments(request, CancellationToken.None);

        Assert.Same(preview, Assert.IsType<OkObjectResult>(result.Result).Value);
        Assert.Equal(request, service.LastGarageAnnualPaymentPreviewRequest);
    }
    ...
    public async Task GarageAnnualPaymentsEndpoints_MapInvalidYearAndMissingGarage()
    {
        ...
        {
            GarageAnnualPaymentsResult = FinanceResult<GarageAnnualPaymentsDto>.Failure(
                "annual_payment_year_invalid",
                "Неверный год.")
        }).GetGarageAnnualPayments(Guid.NewGuid(), 1999, CancellationToken.None);
        var missing = await CreateController(new FakeFinanceService
        {
            GarageAnnualPaymentsResult = FinanceResult<GarageAnnualPaymentsDto>.Failure(
                "garage_not_found",
                "Гараж не найден.")
        }).CalculateGarageAnnualPayments(Guid.NewGuid(), 2026, CancellationToken.None);
    ...
    [Fact]
    public async Task GetExpenseWorksheetStaffBreakdown_AllowsAllEmployeeExpenseTypes()
    {
        var staffMemberId = Guid.NewGuid();
        ...
        Assert.IsType<OkObjectResult>(result.Result);
        Assert.Null(service.LastExpenseWorksheetStaffBreakdownRequest?.ExpenseTypeId);
    }
    ...
    [Fact]
    public async Task UpdateStaffPayment_PassesVersionAndActorUserIdToService()
    {
        var actorUserId = Guid.NewGuid();
        var operationId = Guid.NewGuid();
        var expectedVersion = Guid.NewGuid();
        var request = new UpdateStaffPaymentRequest(Guid.NewGuid(), new DateOnly(2026, 6, 26), new DateOnly(2026, 6, 1), 23000m, "PAY-1", "Исправлено", expectedVersion);
        var service = new FakeFinanceService
        {
            UpdateStaffPaymentResult = FinanceResult<FinancialOperationDto>.Success(CreateOperation("expense", operationId, amount: 23000m))
        };
        var controller = CreateController(service, actorUserId);

        var result = await controller.UpdateStaffPayment(operationId, request, CancellationToken.None);

        Assert.IsType<OkObjectResult>(result.Result);
        Assert.Equal(actorUserId, service.LastActorUserId);
        Assert.Equal(operationId, service.LastUpdatedOperationId);
        Assert.Same(request, service.LastUpdatedStaffPaymentRequest);
        Assert.Equal(expectedVersion, service.LastUpdatedStaffPaymentRequest!.ExpectedVersion);
    }

    [Fact]
    public async Task UpdateStaffPayment_ReturnsForbiddenWhenAdministratorDisabledEditing()
    {
        var service = new FakeFinanceService
        {
            UpdateStaffPaymentResult = FinanceResult<FinancialOperationDto>.Failure(
                "payout_editing_disabled",
                "Редактирование выплат отключено администратором.")
        };
        var controller = CreateController(service, Guid.NewGuid());

        var result = await controller.UpdateStaffPayment(
            Guid.NewGuid(),
            new UpdateStaffPaymentRequest(Guid.NewGuid(), new DateOnly(2026, 6, 26), new DateOnly(2026, 6, 1), 23000m, null, null, Guid.NewGuid()),
            CancellationToken.None);
    ...
    [Fact]
    public async Task CreateStaffSalaryAdjustment_ReturnsCreatedAndPassesActorUserIdToService()
    {
        var actorUserId = Guid.NewGuid();
        var staffMemberId = Guid.NewGuid();
        var month = new DateOnly(2026, 6, 1);
        var request = new CreateStaffSalaryAdjustmentRequest(staffMemberId, month, "bonus", 5000m, "PR-1", "За качественную работу");
        var adjustment = new StaffSalaryAdjustmentDto(
            Guid.NewGuid(),
            staffMemberId,
            "Петрова Ольга",
            month,
        ...
        var service = new FakeFinanceService
        {
            CreateStaffSalaryAdjustmentResult = FinanceResult<StaffSalaryAdjustmentDto>.Success(adjustment)
        };
        var controller = CreateController(service, actorUserId);

        var result = await controller.CreateStaffSalaryAdjustment(request, CancellationToken.None);

        var created = Assert.IsType<CreatedAtActionResult>(result.Result);
        Assert.Equal(nameof(FinanceController.GetExpenseWorksheet), created.ActionName);
        Assert.Same(adjustment, created.Value);
        Assert.Equal(actorUserId, service.LastActorUserId);
        Assert.Same(request, service.LastStaffSalaryAdjustmentRequest);
    }

    [Theory]
    [InlineData("staff_member_not_found", 404)]
    [InlineData("staff_penalty_exceeds_available", 409)]
    [InlineData("staff_salary_adjustment_type_invalid", 400)]
    public async Task CreateStaffSalaryAdjustment_MapsServiceErrors(string errorCode, int expectedStatus)
    {
        var controller = CreateController(new FakeFinanceService
        {
            CreateStaffSalaryAdjustmentResult = FinanceResult<StaffSalaryAdjustmentDto>.Failure(errorCode, "Ошибка")
        });

        var result = await controller.CreateStaffSalaryAdjustment(
            new CreateStaffSalaryAdjustmentRequest(Guid.NewGuid(), new DateOnly(2026, 6, 1), "bonus", 100m, null, "Основание"),
            CancellationToken.None);
    ...
    [Fact]
    public async Task UpdateStaffSalaryAdjustment_ReturnsUpdatedRecord()
    {
        var adjustmentId = Guid.NewGuid();
        var version = Guid.NewGuid();
        var request = new UpdateStaffSalaryAdjustmentRequest(Guid.NewGuid(), new DateOnly(2026, 6, 1), "bonus", 500m, null, "Основание", version);
        var adjustment = new StaffSalaryAdjustmentDto(adjustmentId, request.StaffMemberId, "Петрова Ольга", request.AccountingMonth, "bonus", 500m, null, "Основание", Version: Guid.NewGuid());
        var controller = CreateController(new FakeFinanceService
        {
            UpdateStaffSalaryAdjustmentResult = FinanceResult<StaffSalaryAdjustmentDto>.Success(adjustment)
        });

        var result = await controller.UpdateStaffSalaryAdjustment(adjustmentId, request, CancellationToken.None);

        Assert.Same(adjustment, Assert.IsType<OkObjectResult>(result.Result).Value);
    }

    [Fact]
    public async Task CancelStaffSalaryAdjustment_MapsVersionConflict()
    {
        var controller = CreateController(new FakeFinanceService
        {
            CancelStaffSalaryAdjustmentResult = FinanceResult<StaffSalaryAdjustmentDto>.Failure("staff_salary_adjustment_version_conflict", "Конфликт")
        });

        var result = await controller.CancelStaffSalaryAdjustment(
            Guid.NewGuid(),
            new CancelStaffSalaryAdjustmentRequest("Ошибка", Guid.NewGuid()),
            CancellationToken.None);
    ...
    [Fact]
    public async Task RestoreStaffSalaryAdjustment_MapsMissingRecord()
    {
        var controller = CreateController(new FakeFinanceService
        {
            RestoreStaffSalaryAdjustmentResult = FinanceResult<StaffSalaryAdjustmentDto>.Failure("staff_salary_adjustment_not_found", "Не найдено")
        });

        var result = await controller.RestoreStaffSalaryAdjustment(
            Guid.NewGuid(),
            new RestoreStaffSalaryAdjustmentRequest(Guid.NewGuid()),
            CancellationToken.None);
    ...
    [Fact]
    public async Task CreateMeterReading_AllowsAnotherMonthWithoutReason()
    {
        var service = new FakeFinanceService
        {
            CreateMeterReadingResult = FinanceResult<MeterReadingDto>.Success(CreateMeterReading())
    ...
    [Fact]
    public async Task UpdateMeterReading_AllowsAnotherMonthWithoutReason()
    {
        var meterReadingId = Guid.NewGuid();
        var service = new FakeFinanceService
        {
            UpdateMeterReadingResult = FinanceResult<MeterReadingDto>.Success(CreateMeterReading(id: meterReadingId))
        };
        var controller = CreateController(
            service,
    ...
    [Fact]
    public async Task SavePaymentFormMeterReading_AllowsAnotherMonthWithoutReason()
    {
        var service = new FakeFinanceService
        {
            SavePaymentFormMeterReadingResult = FinanceResult<MeterReadingDto>.Success(CreateMeterReading())
        };
        var controller = CreateController(
            service,
            businessDate: new DateOnly(2026, 7, 17),
    ...
    private sealed class FakeFinanceService : IFinanceService
    {
        ...
        public UpdateStaffPaymentRequest? LastUpdatedStaffPaymentRequest { get; private set; }
        ...
        public GarageAnnualPaymentPreviewRequest? LastGarageAnnualPaymentPreviewRequest { get; private set; }
        ...
        public FinanceResult<GarageAnnualPaymentPreviewDto> GarageAnnualPaymentPreviewResult { get; init; } = FinanceResult<GarageAnnualPaymentPreviewDto>.Failure("not_configured", "Not configured.");
        ...
        public FinanceResult<FinancialOperationDto> UpdateStaffPaymentResult { get; init; } = FinanceResult<FinancialOperationDto>.Failure("not_configured", "Not configured.");
        ...
        public FinanceResult<StaffSalaryAdjustmentDto> UpdateStaffSalaryAdjustmentResult { get; init; } = FinanceResult<StaffSalaryAdjustmentDto>.Failure("not_configured", "Not configured.");
        public FinanceResult<StaffSalaryAdjustmentDto> CancelStaffSalaryAdjustmentResult { get; init; } = FinanceResult<StaffSalaryAdjustmentDto>.Failure("not_configured", "Not configured.");
        public FinanceResult<StaffSalaryAdjustmentDto> RestoreStaffSalaryAdjustmentResult { get; init; } = FinanceResult<StaffSalaryAdjustmentDto>.Failure("not_configured", "Not configured.");
        ...
        public Task<FinanceResult<GarageAnnualPaymentsDto>> GetGarageAnnualPaymentsAsync(
            Guid garageId,
            int year,
            CancellationToken cancellationToken)
        {
            ...
            CancellationToken cancellationToken)
        {
            LastGarageAnnualPaymentsGarageId = garageId;
            LastGarageAnnualPaymentsYear = year;
            LastGarageAnnualPaymentsActorUserId = actorUserId;
            return Task.FromResult(GarageAnnualPaymentsResult);
        }

        public Task<FinanceResult<GarageAnnualPaymentPreviewDto>> PreviewGarageAnnualPaymentsAsync(
            GarageAnnualPaymentPreviewRequest request,
            CancellationToken cancellationToken)
        {
            LastGarageAnnualPaymentPreviewRequest = request;
            return Task.FromResult(GarageAnnualPaymentPreviewResult);
        }
        ...
        public Task<FinanceResult<FinancialOperationDto>> UpdateStaffPaymentAsync(Guid operationId, UpdateStaffPaymentRequest request, Guid? actorUserId, CancellationToken cancellationToken)
        {
            LastActorUserId = actorUserId;
            LastUpdatedOperationId = operationId;
            LastUpdatedStaffPaymentRequest = request;
            return Task.FromResult(UpdateStaffPaymentResult);
        }

        public Task<FinanceResult<StaffSalaryAdjustmentDto>> CreateStaffSalaryAdjustmentAsync(CreateStaffSalaryAdjustmentRequest request, Guid? actorUserId, CancellationToken cancellationToken)
        {
            LastActorUserId = actorUserId;
        ...
        public Task<FinanceResult<StaffSalaryAdjustmentDto>> UpdateStaffSalaryAdjustmentAsync(Guid adjustmentId, UpdateStaffSalaryAdjustmentRequest request, Guid? actorUserId, CancellationToken cancellationToken)
        {
            LastActorUserId = actorUserId;
            return Task.FromResult(UpdateStaffSalaryAdjustmentResult);
        }

        public Task<FinanceResult<StaffSalaryAdjustmentDto>> CancelStaffSalaryAdjustmentAsync(Guid adjustmentId, CancelStaffSalaryAdjustmentRequest request, Guid? actorUserId, CancellationToken cancellationToken)
        {
            LastActorUserId = actorUserId;
            return Task.FromResult(CancelStaffSalaryAdjustmentResult);
        }

        public Task<FinanceResult<StaffSalaryAdjustmentDto>> RestoreStaffSalaryAdjustmentAsync(Guid adjustmentId, RestoreStaffSalaryAdjustmentRequest request, Guid? actorUserId, CancellationToken cancellationToken)
        {
            LastActorUserId = actorUserId;
            return Task.FromResult(RestoreStaffSalaryAdjustmentResult);
        }
