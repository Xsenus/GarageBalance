...
23.09.2026: владелец показал новый пустой бакет Cloud.ru Evolution Object Storage `sgk-garagebalance` (холодный класс). Сверены официальные параметры S3: `https://s3.cloud.ru`, регион `ru-central-1`, состав идентификатора доступа и роли сервисного аккаунта. В `docs/storage-operations.md` добавлена неактивная заготовка подключения и последовательность безопасной проверки. Рабочий доступ/доставка не включены: вкладка Cloud.ru недоступна в текущей сессии, ключ S3/целевая установка не предоставлены; реальный тест совместимости SSE-S3 и подписанных операций не выполнен. Один бакет покрывает лишь одно внешнее назначение. `TASK-021..023` остаются открытыми; документация проверена на корректный JSON и `git diff --check`, приложение не менялось, поэтому тесты/БД не запускались.

23.09.2026: владелец разрешил временный локальный тест с предоставленным персональным ключом Cloud.ru; секрет не сохранялся в файлах или конфигурации. Реальный `PutObject` в пустой тестовый путь бакета отклонён Cloud.ru из-за SSE-S3 `AES256`, поэтому рабочие резервные копии не отправлялись. Для S3-адаптера добавлен выбираемый SSE-KMS с ID симметричного ключа, проверка обязательного ID и запрет SSE-S3 для `s3.cloud.ru`; default SSE-S3 для других провайдеров сохранён. Интеграционный тест умеет проверять существующий бакет без его создания/удаления и удаляет только уникальный тестовый объект. Профильные backend-тесты: 32 passed, 0 failed, 1 skipped до появления KMS-ключа; `dotnet format --verify-no-changes` для изменённых C# файлов прошёл. Владелец разрешил создание KMS-ключа; для API требуется ID экземпляра Key Management, запрошен адрес страницы KMS. Режим `Single` не менялся, Cloud.ru не подключён к рабочим копиям, `TASK-021..023` остаются внешней приёмкой. Публично раскрытый персональный ключ необходимо отозвать после временных тестов и заменить ограниченным ключом сервисного аккаунта.

23.09.2026: владелец создал активный симметричный KMS-ключ для backup. Реальный тест с временным персональным доступом подтвердил SSE-KMS для малого объекта. Расширенный тест multipart выявил, что Cloud.ru требует SHA-256 каждой части в `CompleteMultipartUpload`; адаптер исправлен и защищён unit-регрессией. После исправления реальный цикл обычного и multipart-объектов прошёл: запись, `HEAD`, чтение с побайтовой сверкой, подписанная ссылка, удаление и повторная проверка отсутствия; только уникальные тестовые объекты, без дампов и без изменения конфигурации приложения. Профильные unit-тесты S3: 14 passed; реальный интеграционный тест: 1 passed; весь затронутый набор без внешних реквизитов: 32 passed, 1 expected skip. Форматирование изменённых C# файлов, UTF-8 и `git diff --check` прошли; task-owned .NET-процессы остановлены. Сервисный аккаунт, ротация опубликованного персонального ключа, рабочая репликация, DR и нагрузочная проверка ещё не выполнены; `TASK-021..023` остаются открытыми.

23.09.2026: владелец уточнил границу эксплуатации: этот ПК — только тестовый стенд, автоматическую доставку и VPS нужно настраивать после выпуска обновления; временно разрешено пользоваться ранее выданным персональным ключом. Из локальной `garagebalance_local` (0 гаражей) штатным `pg_dump` создан новый custom-format архив в приватной папке профиля, полностью прочитан `pg_restore --list` и `pg_restore --file=NUL`. Архив загружен в `sgk-garagebalance` с SSE-KMS под отдельным `local-test` префиксом, считан обратно и побайтово сверен по размеру и SHA-256; проверенная локальная и облачная копии сохранены как результат теста. Автоматический процесс/задача на тестовом ПК не создавались, VPS и рабочие дампы не затрагивались. Полное восстановление в отдельную БД здесь не подтверждено: локальная роль не имеет `CREATEDB`, а запуск временного PostgreSQL отклонён политикой среды. Перед включением VPS после релиза остаются миграции рабочей БД, защищённая передача/хранение реквизитов, настройка `AsyncMirror`, проверка реального рабочего дампа и отдельный restore-drill; `TASK-021..023` остаются открытыми.

23.09.2026: перед интеграцией выполнены `fetch --all --prune`, проверка веток и PR: только `master` и `origin/master`, 13 локальных коммитов поверх удалённой ветки, конфликтов и невлитых рабочих веток нет. Устаревший незакоммиченный аудит по выбору владельца перенесён без изменения содержимого в личный архив вне Git. На отдельном временном PostgreSQL 17 с ICU полный backend Release прошёл 3171/3172 (1 ожидаемый пропуск реального S3), coverage 90,02% строк и 75,18% ветвей; проверка модели EF и генерация идемпотентного SQL прошли. На отдельной чистой БД применены все 149 миграций, API запущен: `/health` = 200, а защищённый маршрут без авторизации = 401; временная тестовая резервная копия удалена. Release-сборка backend без предупреждений, форматирование, аудит зависимостей, privacy-сканирование и статическая проверка Docker-дистрибутива прошли; Docker CLI на ПК отсутствует, контейнерный запуск оставлен CI. Frontend с `f4b0e0ad` не менялся: сохранён успешный полный прогон 1310/1310 и coverage 88,61/81,47/87,06/89,79%; текущие lint, production build, bundle budget и npm audit прошли. Избыточные повторные полные frontend-прогоны остановлены после тайм-аутов на ПК под параллельной нагрузкой другого проекта; их результат не заявлен как успешный. Push, CI/CD и VPS-проверка на момент записи ещё не выполнены. `TASK-021..023` остаются открытыми до независимого DR, настройки рабочей репликации и испытаний на реальных данных.

23.09.2026: `master/c9fb4eb7` отправлен на GitHub и успешно развернут в staging workflow: backend 3171 passed / 1 expected skip, frontend 1310 passed, аудит/форматирование зелёные; деплой создал и проверочно восстановил backup (51 таблица), применил миграции, проверил nginx и запустил сервис. Публичные `/`, JS-артефакт и `/health` отвечают 200, PostgreSQL `ok`, защищённый API без авторизации отвечает 401, экран входа открылся без ошибок консоли. Дополнительный ручной `Verify branch` выявил дефект самого quality job: shallow checkout глубиной 1 превращал `git show --check HEAD` в проверку всей истории как корневого коммита и находил старые пробелы Markdown. Для корректной проверки текущего коммита задан `fetch-depth: 2`; повторный CI и Docker gate на момент записи ещё ожидаются.

23.09.2026: `e4444929` исправил CI quality job и успешно прошёл повторную staging-публикацию. Отдельный `Verify branch` подтвердил backend/frontend/quality, но Docker-установка на чистой БД выявила реальный дефект: предмиграционный backup обращался к каталогу `storage_objects` до создания таблиц и останавливал API. Startup исправлен: пустая база без таблиц проходит миграции без бессмысленной копии, а при существующих таблицах обязательная копия по-прежнему блокирует миграцию при ошибке. Регрессия на PostgreSQL 17: 3/3 профильных теста, живой запуск на новой БД применил 149 миграций и `/health` = 200; полный backend 3173 passed, 0 failed, 1 expected external S3 skip, coverage 90,04% строк и 75,2% ветвей. Форматирование, release-тесты 48/48, Docker-контракты 6/6 прошли; руководство первой установки и «Что нового» обновлены. Повторный Docker CI и публикация этого исправления на момент записи ещё ожидаются.

23.09.2026: владелец разрешил временно настроить staging-VPS с теми же персональными Cloud.ru S3-реквизитами, которые прошли локальный SSE-KMS тест. Подготовлены ограниченный root-помощник без передачи секрета в аргументах, защищённый systemd env/drop-in, откат при неуспешном старте API, доставка StorageTool в релизе и команды инвентаризации/переноса/проверки исторических дампов. Профильные deployment-тесты 12/12, Linux `bash -n` трёх скриптов и локальная linux-x64 публикация StorageTool прошли. На момент записи Cloud.ru на VPS ещё **не включён**, реальный перенос и DR не проверены; `TASK-021..023` не закрывать. Опубликованный персональный ключ после запуска подлежит ротации на ограниченный сервисный.

23.09.2026: релиз `e049429b` прошёл полный CI/Docker и установлен на staging. Перед подключением инвентаризация VPS показала 81 локальный `.pgdump`, 62 054 737 байт и `Storage=Single`. Первая попытка включения с разрешённым персональным ключом откатилась штатно: мгновенная внешняя проверка получила временный HTTP 502 сразу после `systemctl restart`; повторная инспекция подтвердила `Single` и активный API. Для повторной попытки проверка исправлена на ограниченное ожидание готовности с повторными запросами, обработчик ошибки явно возвращает ненулевой код. Реальная облачная защита и перенос ещё не подтверждены.

23.09.2026: повторный релиз `10808276` успешно прошёл CI и установлен. `AsyncMirror` включился и `/health/ready` стал здоровым; VPS подтвердил 81 локальный архив. `inventory` и `plan` завершились с кодом 0, но первый `copy` возвратил код 10 (проверочные blockers); источники не удалены и успех защиты не заявлен. Для диагностики добавлен узко ограниченный вывод последних строк журнала только storage-tool unit; после публикации потребуется выяснить blocker, продолжить из checkpoint и выполнить `verify`/`cutover-check`.

23.09.2026: `3f798a12` и `7f03ee8c` прошли staging CI, последний дал полный отчёт StorageTool. На VPS найдено 81 локальный `.pgdump`; первый перенос зарегистрировал 29 архивов, 40 исторических/предрелизных имен отклонены строгим старым форматом, 12 архивов без префикса `garagebalance_` не входили в обход. Контрольная точка `pending`, 15 заданий обработки, `cutover-check` ещё не проходил. Исправлен обход всех `.pgdump` без молчаливых пропусков; добавлено точное распознавание известных исторических и предрелизных имён, а неизвестные имена продолжают блокировать приёмку. На время Cloud-режима приостановлена независимая очистка предрелизных файлов в deploy-скрипте; подготовлен ежечасный `delta-sync` после первичной приёмки. Профильные тесты 30/30, оба Bash-скрипта прошли `bash -n`; до публикации и реальной проверки новые архивы/Cloud-защита не объявляются завершёнными.

23.09.2026: `98c021be` прошёл полный staging workflow и установился; инвентаризация после предрелизного backup видит 82 архива: 65 проверены, 12 иных исторических названий ещё не распознаны, 5 архивов дают `unreadable_archive_or_manifest`. Для 12 добавлено строгое правило известных имён; для пяти подготовлена закрытая диагностика владельца/прав/размера файла и sidecar без чтения содержимого или вывода секретов. Профильные тесты 34/34; до расследования пяти и положительного `cutover-check` облачную защиту полной не считать.

23.09.2026: `a6f72c2f` прошёл полный staging workflow; после нового предрелизного дампа инвентаризация охватила 83 архива. Выяснено, что 7 старых архивов недоступны процессу `garagebalance`: шесть принадлежат `postgres`, один `root`, все имеют режим `0600`; sidecar у них нет. Остальные 76 уже зарегистрированы через `delta-sync`, контрольная точка `copied_requires_verification`, источники не удалены. Для семи подготовлена строго ограниченная операция проверки `pg_restore --list` с выдачей лишь группового чтения (`0640`) без смены владельца; профильные deployment-тесты 4/4. После следующей установки требуются повторный перенос, физическая проверка Cloud.ru и `cutover-check`.

23.09.2026: `0585a35e` прошёл все пять заданий staging workflow и установлен. Команда `allow-legacy-read` проверила семь точных старых архивов через `pg_restore --list` и предоставила процессу backup лишь групповое чтение. После установки инвентаризация показала 84/84 проверенных архивов без ошибок. `delta-sync`, `verify` и `cutover-check` на настоящем Cloud.ru завершились с кодом 0, `AsyncMirror` и API активны. Ежечасный systemd-таймер синхронизации создан и включён. Предел приёмки: полное восстановление при потере VPS с ключами Data Protection, ротация опубликованного персонального S3-ключа, независимый DR drill, нагрузка/долговременное наблюдение и второй внешний домен отказа остаются открытыми по `TASK-021..023`; успешная проверка 84 копий не закрывает их автоматически.

24.09.2026: владелец предоставил второй бакет HOSTKEY NL и отдельно разрешил хранить там копию без шифрования. По официальной документации подтверждены endpoint `https://s3-nl.hostkey.com`, регион `nl`, отдельные Access/Secret Key (S3UID не используется для подписи). Реальный тест с уникальными объектами показал: обычная запись, checksum и `If-None-Match` работают; SSE-S3 возвращает `400 InvalidArgument`, SSE-C — `400 InvalidRequest`. Все тестовые объекты удалены. Подготовлены per-destination реквизиты только из закрытого environment, явное opt-in к отсутствию шифрования, третье независимое назначение и новая контрольная точка backfill с откатом настройки при неуспешном старте. Профильные unit/deployment-тесты 45/45, реальный обычный+multipart S3 round-trip 1/1. На момент записи изменения ещё **не опубликованы**, Hostkey на VPS не подключён, `TASK-021..023` остаются открытыми; после публикации обязательны настоящий backfill и полный `cutover-check` всех архивов.

24.09.2026: полный backend-прогон с локальным PostgreSQL 17 завершился: 3200 пройдено, 1 пропущен, ошибок нет; покрытие 90,07% строк и 75,17% ветвей при порогах 85%/70%. Источник выпуска проверен 18/18 тестами. Подготовлена запись «Что нового» о третьей копии и явное предупреждение об отсутствии шифрования на стороне HOSTKEY. Публикация и VPS-проверка выполняются следующим этапом; их результат нельзя считать успешным до реального backfill и `cutover-check`.

24.09.2026: коммит `94d8eaf2` опубликован в `master`, staging workflow `35950206045` завершился успешно (backend/frontend тесты, аудит, форматирование, миграционный SQL, сборка и развёртывание). На VPS установлено третье назначение HOSTKEY NL с секретами только в закрытом `/etc/garagebalance-staging-hostkey.env` (режим `0600`); часовой таймер был выключен на время первоначальной загрузки. Инвентаризация и план нашли 87 архивов без блокировок; `copy` перенёс исторические копии, а `delta-sync` завершил отложенную доставку свежего предрелизного архива. После этого `verify` и `cutover-check` завершились с кодом 0, `status` показал `Protected: 87` из `catalogObjects: 87`, без паузы reconciliation. Таймер снова включён; приложение и nginx активны, `/health/ready` и frontend отвечают HTTP 200. HOSTKEY хранит копии в закрытом бакете без шифрования на стороне провайдера по явному согласию владельца; это не закрывает независимый DR drill, хранение ключей восстановления и наблюдение из `TASK-021..023`.
