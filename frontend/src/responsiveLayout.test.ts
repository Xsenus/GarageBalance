...
describe('responsive layout styles', () => {
  ...
  it('keeps configurable tariff panels level on desktop and compact laptops, then stacks them on narrow screens', () => {
    ...
    expect(normalizedAppCss).toContain('.irregular-payments-table-scroll,\n.fee-campaign-table-scroll {\n  min-height: 0;\n  flex: 1 1 auto;\n  overflow-x: auto;')
    ...
    expect(compactDesktopCss).toContain('.irregular-payments-table-scroll,\n  .fee-campaign-table-scroll {\n    overflow-x: auto;')
  ...
  it('keeps backup and diagnostic status cards readable in settings', () => {
    expect(normalizedAppCss).toContain('.settings-card--diagnostics {\n  grid-template-columns: minmax(280px, 0.65fr) minmax(0, 1fr);')
  ...
  it('keeps the single garage search and grouped active card responsive', () => {
    ...
    expect(normalizedAppCss).toContain('.payments-prototype-balance-explanation {\n  display: flex;\n  align-items: center;\n  justify-content: space-between;')
    expect(normalizedAppCss).toContain('.payments-prototype-overdue-trigger {\n  flex: 0 0 auto;\n  border-color: #fda29b;\n  background: #fff1f0;\n  color: #b42318;')
    expect(normalizedAppCss).toContain('.detail-dialog.garage-overdue-debt-dialog {\n  width: min(1120px, calc(100vw - 32px));')
  ...
  it('keeps the signed garage balance column and four period totals readable', () => {
    ...
    expect(normalizedAppCss).toContain('.payments-prototype-meter-editor {\n  display: grid;\n  grid-template-columns: minmax(0, 1fr) 30px;')
    expect(normalizedAppCss).toContain('.payments-prototype-payment-input,\n.payments-prototype-meter-input {\n  box-sizing: border-box;')
  ...
  it('keeps payment history columns compact, centered and readable', () => {
    ...
    expect(normalizedAppCss).toContain('.payments-prototype-mini-table th:nth-child(6),\n.payments-prototype-mini-table td:nth-child(6) {\n  width: 204px;')
    expect(normalizedAppCss).toContain('.payments-prototype-mini-table th:last-child,\n.payments-prototype-mini-table td:last-child {\n  width: 92px;\n  padding-inline: 8px;\n  text-align: center;')
  ...
  it('widens the owner editor and keeps its identity fields responsive', () => {
    ...
    expect(normalizedAppCss).toContain('.full-grid-row {\n  grid-column: 1 / -1;')
  ...
  it('keeps the yearly meter table inside the desktop workspace', () => {
    ...
    expect(normalizedAppCss).toContain('.meter-readings-data-row > span:first-child {\n  position: sticky;\n  left: 0;\n  z-index: 2;\n  display: flex;')
  ...
  it('keeps settings navigation full-height and settings forms compact', () => {
    ...
    expect(normalizedAppCss).toContain('.settings-card--backups .settings-card-body {\n  display: flex;\n  min-height: 0;\n  flex: 1;\n  flex-direction: column;')
    expect(normalizedAppCss).toContain('.settings-backup-table-shell {\n  flex: 1;\n  overflow: auto;')
  ...
  it('keeps the garage editor wide, compact and responsive', () => {
    ...
    expect(normalizedAppCss).toContain('.contractors-garage-form-comment {\n  display: grid;\n  grid-column: 1 / -1;\n  grid-template-rows: auto minmax(52px, 1fr);')
  ...
  it('uses a compact payments workspace on the customer laptop viewport', () => {
    ...
    expect(normalizedAppCss).toContain('.payments-prototype-sheet--income > .payments-prototype-period-toolbar {\n    grid-template-columns: minmax(620px, 1.45fr) minmax(440px, 1fr);')
    expect(normalizedAppCss).toContain('.payments-prototype-sheet--income .payments-prototype-period-row {\n    display: grid;\n    grid-template-columns: 100px 100px minmax(0, 1fr);\n    align-items: end;')
  ...
  it('keeps the supplier editor wide, compact and responsive', () => {
    ...
    expect(normalizedAppCss).toContain("[data-required='true'] > .form-field-label) .form-field-label-text::after {\n  color: #f04438;\n  content: ' *';")
    expect(normalizedAppCss).not.toContain('> .form-field-label:has(> .field-label-with-help)::after')
