...
describe('reportsApi', () => {
  ...
  it('downloads report exports through POST because the backend records audit events', async () => {
    ...
    await reportsApi.exportFeeReportXlsx('token', { feeEntryIds: ['fee-1', 'fee-2'] })
    await reportsApi.exportFeeReportPdf('token', { feeEntryIds: ['fee-1', 'fee-2'] })
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(11, '/api/reports/fees/export/xlsx?feeEntryIds=fee-1&feeEntryIds=fee-2', postRequest())
    expect(fetchMock).toHaveBeenNthCalledWith(12, '/api/reports/fees/export/pdf?feeEntryIds=fee-1&feeEntryIds=fee-2', postRequest())
  ...
  it('loads paged garage, income, expense, cash, bank and fund reports with other dedicated filtered endpoints', async () => {
    ...
    await reportsApi.getFeeReport('token', { feeEntryIds: ['fee-1', 'fee-2'], limit: 20 })
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(5, '/api/reports/fees?feeEntryIds=fee-1&feeEntryIds=fee-2&limit=20', getRequest())
