...
describe('shared validation helpers', () => {
  ...
  it('validates garage, supplier and accounting type forms', () => {
    ...
    expect(getAccountingTypeValidationErrors({ name: '', code: 'код', isSystem: false }, 'вида поступления')).toEqual([
      ...
      'Код вида поступления должен начинаться с латинской буквы и содержать только латинские буквы, цифры и знак подчёркивания.',
    ])
    expect(getAccountingTypeValidationErrors({ name: 'Охрана', code: '1_security', isSystem: false }, 'вида поступления')).toEqual([
      'Код вида поступления должен начинаться с латинской буквы и содержать только латинские буквы, цифры и знак подчёркивания.',
    ])
    expect(getAccountingTypeValidationErrors({ name: 'Охрана', code: `a${'b'.repeat(80)}`, isSystem: false }, 'вида поступления')).toEqual([
      'Код вида поступления не должен превышать 80 символов.',
    ])
  })
  ...
  it('validates finance operation and accrual forms', () => {
    ...
    expect(getExpenseValidationErrors({ supplierId: '', expenseTypeId: '', expensePaymentType: 'invalid' as never, expensePaymentSource: 'invalid' as never, expenseFundId: '', operationDate: 'bad', accountingMonth: 'bad', amount: -1, documentNumber: '', comment: '' })).toEqual([
      ...
      'Для поставщика должна быть настроена услуга.',
      'Для услуги поставщика должен быть настроен фонд расходования.',
    ...
    expect(getSupplierAccrualValidationErrors({ supplierId: '', expenseTypeId: '', accountingMonth: 'bad', amount: 0, source: 'manual', documentNumber: '', comment: '' })).toEqual([
      ...
      'Для поставщика должна быть настроена услуга начисления.',
  ...
  it('validates regular accruals, salary accruals and meter readings', () => {
    ...
    expect(getMeterReadingValidationErrors({
      garageId: 'garage-1',
      meterKind: 'service_123456781234123412341234567890ab',
