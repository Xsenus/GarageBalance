...
describe('shared formatters', () => {
  ...
  it('formats import and accrual statuses', () => {
    ...
    expect(formatImportRunStatus('queued')).toBe('В очереди')
    expect(formatImportRunStatus('processing')).toBe('Выполняется')
    expect(formatImportRunStatus('failed')).toBe('Ошибка')
