...
describe('TablePagination', () => {
  ...
  it('uses a one-line page-size combobox in compact table pagination', () => {
    ...
    const pageSizeControl = screen.getByRole('combobox', { name: 'Количество строк' })
    fireEvent.click(pageSizeControl)
    expect(pageSizeControl).toHaveAttribute('aria-expanded', 'true')
    expect(screen.getByRole('listbox', { name: 'Количество строк: варианты' })).toHaveClass('select-control__list--above')
    expect(screen.getAllByRole('option')).toHaveLength(4)
    fireEvent.click(screen.getByRole('option', { name: '25' }))
    expect(onPageSizeChange).toHaveBeenCalledWith(25)
    expect(pageSizeControl).toHaveAttribute('aria-expanded', 'false')
