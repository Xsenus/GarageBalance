...
describe('App', () => {
  ...
  it('shows meter readings prototype as a yearly garage table', async () => {
    ...
    expect(within(readingConfirmDialog).getByText('Поле')).toBeInTheDocument()
    expect(within(readingConfirmDialog).getByText('Показание')).toBeInTheDocument()
    expect(within(readingConfirmDialog).getByText('Было')).toBeInTheDocument()
    ...
    expect(within(readingConfirmDialog).getByText('4660')).toBeInTheDocument()
  ...
  it('shows funds management prototype from dashboard tile', async () => {
    ...
    await waitFor(() => expect(depositConfirmButton).toHaveFocus())
    await user.keyboard('{Tab}')
