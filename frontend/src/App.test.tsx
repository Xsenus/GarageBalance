...
describe('App', () => {
  ...
  it('edits tariffs and one-time payments without local history access', async () => {
    ...
    await user.click(within(waterRateConfirmDialog).getByRole('button', { name: 'Сохранить изменение' }))
  ...
  it('shows funds management prototype from dashboard tile', async () => {
    ...
    await user.click(within(fundsPanel).getByRole('button', { name: 'Изъять из фонда Электроэнергия' }))
    ...
    await user.click(within(depositDialog).getByRole('button', { name: 'Подтвердить операцию' }))
    expect(within(depositDialog).getByRole('alert')).toHaveTextContent('Укажите сумму больше нуля.')
    await user.type(within(depositDialog).getByLabelText('Сумма операции фонда'), '1500')
    await user.type(within(depositDialog).getByLabelText('Причина операции фонда'), 'Распределение средств')
    expect(within(depositDialog).getByText(/1[\s\u00A0]500,00 руб\./)).toBeInTheDocument()
    await user.click(within(depositDialog).getByRole('button', { name: 'Подтвердить операцию' }))
