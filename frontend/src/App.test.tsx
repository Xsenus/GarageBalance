...
describe('App', () => {
  ...
  it('edits tariffs and one-time payments without local history access', async () => {
    ...
    const waterRateConfirmButton = within(waterRateConfirmDialog).getByRole('button', { name: 'Сохранить изменение' })
    ...
    expect(waterRateInput).toHaveValue('')

    await user.type(waterRateInput, '1250{Enter}')
    const reopenedWaterRateConfirmDialog = await screen.findByRole('dialog', { name: 'Подтвердить изменение?' })
    await user.click(within(reopenedWaterRateConfirmDialog).getByRole('button', { name: 'Сохранить изменение' }))
    ...
    const thresholdConfirmDialog = await screen.findByRole('dialog', { name: 'Подтвердить изменение?' })
    expect(within(thresholdConfirmDialog).getByText('Электроэнергия: Порог 4')).toBeInTheDocument()
    await user.click(within(thresholdConfirmDialog).getByRole('button', { name: 'Сохранить изменение' }))
    ...
    await user.click(within(entryFeeConfirmDialog).getByRole('button', { name: 'Сохранить изменение' }))
    ...
    const deleteFineConfirmButton = within(deleteFineDialog).getByRole('button', { name: 'Удалить нерегулярный платеж' })
    ...
    const deleteFineReasonInput = within(deleteFineDialog).getByLabelText('Причина удаления нерегулярного платежа')
    expect(deleteFineConfirmButton).toBeDisabled()
    await waitFor(() => expect(deleteFineCancelButton).toHaveFocus())
    await user.keyboard('{Shift>}{Tab}{/Shift}')
    expect(deleteFineReasonInput).toHaveFocus()
    await user.keyboard('{Shift>}{Tab}{/Shift}')
    expect(deleteFineCloseButton).toHaveFocus()
    await user.keyboard('{Tab}')
    expect(deleteFineReasonInput).toHaveFocus()
    await user.keyboard('{Tab}')
    expect(deleteFineCancelButton).toHaveFocus()
    await user.keyboard('{Tab}')
    expect(deleteFineCloseButton).toHaveFocus()
    await user.keyboard('{Escape}')
    await waitFor(() => expect(screen.queryByRole('dialog', { name: 'Удалить нерегулярный платеж?' })).not.toBeInTheDocument())
    await waitFor(() => expect(deleteFineButton).toHaveFocus())

    await user.click(deleteFineButton)
    const reopenedDeleteFineDialog = await screen.findByRole('dialog', { name: 'Удалить нерегулярный платеж?' })
    expect(within(reopenedDeleteFineDialog).getByRole('button', { name: 'Удалить нерегулярный платеж' })).toBeDisabled()
    ...
    await user.click(within(reopenedDeleteFineDialog).getByRole('button', { name: 'Удалить нерегулярный платеж' }))
    ...
    expect(within(tariffsPanel).getByText('Удален')).toBeInTheDocument()
    const restoreFineCancelButton = within(restoreFineDialog).getByRole('button', { name: 'Отмена' })
    const restoreFineConfirmButton = within(restoreFineDialog).getByRole('button', { name: 'Вернуть платеж' })
    const restoreFineCloseButton = within(restoreFineDialog).getByRole('button', { name: 'Закрыть подтверждение восстановления нерегулярного платежа' })
    await waitFor(() => expect(restoreFineCancelButton).toHaveFocus())
    await user.keyboard('{Shift>}{Tab}{/Shift}')
    expect(restoreFineCloseButton).toHaveFocus()
    await user.keyboard('{Tab}')
    expect(restoreFineCancelButton).toHaveFocus()
    await user.keyboard('{Tab}')
    expect(restoreFineConfirmButton).toHaveFocus()
    await user.keyboard('{Escape}')
    expect(screen.queryByRole('dialog', { name: 'Вернуть нерегулярный платеж?' })).not.toBeInTheDocument()
    expect(within(tariffsPanel).getByText('Удален')).toBeInTheDocument()
    expect(restoreFineButton).toHaveFocus()

    await user.click(restoreFineButton)
    const reopenedRestoreFineDialog = await screen.findByRole('dialog', { name: 'Вернуть нерегулярный платеж?' })
    await user.click(within(reopenedRestoreFineDialog).getByRole('button', { name: 'Вернуть платеж' }))
  ...
  it('shows contractors tabs and section dialogs without local history access', async () => {
    ...
    const restoreGarageButton = within(garageRow as HTMLElement).getByRole('button', { name: 'Вернуть' })
    ...
    expect(within(garageRow as HTMLElement).getByRole('button', { name: 'Открыть' })).toBeInTheDocument()
    ...
    const supplierDeleteButton = within(editSupplierDialog).getByRole('button', { name: 'Удалить поставщика' })
    await user.click(supplierDeleteButton)
    ...
    expect(supplierDeleteButton).toHaveFocus()
    await user.click(supplierDeleteButton)
    ...
    await user.click(within(supplierRow as HTMLElement).getByRole('button', { name: 'Вернуть' }))
    ...
    expect(within(supplierRow as HTMLElement).getByRole('button', { name: 'Открыть' })).toBeInTheDocument()
    ...
    await user.click(within(updatedEmployeeRow as HTMLElement).getByRole('button', { name: 'Вернуть' }))
    ...
    expect(within(updatedEmployeeRow as HTMLElement).getByRole('button', { name: 'Открыть' })).toBeInTheDocument()
  ...
  it('shows meter readings prototype as a yearly garage table', async () => {
    ...
    const readingSaveButton = within(readingConfirmDialog).getByRole('button', { name: 'Сохранить показание' })
    ...
    expect(januaryInput).toHaveValue('')

    await user.type(januaryInput, '4654{Enter}')
    const reopenedReadingConfirmDialog = await screen.findByRole('dialog', { name: 'Подтвердить показание?' })
    await user.click(within(reopenedReadingConfirmDialog).getByRole('button', { name: 'Сохранить показание' }))
  ...
  it('opens user edit and delete operations from context menu modals', async () => {
    ...
    await waitFor(() => expect(deleteCloseButton).toHaveFocus())
    await user.keyboard('{Tab}')
    ...
    await user.type(deleteReasonInput, 'Access no longer needed')
