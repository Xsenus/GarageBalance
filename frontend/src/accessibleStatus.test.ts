...
describe('accessible dynamic messages', () => {
  ...
  it('keeps required garage fields visibly and accessibly distinguishable while they are filled', () => {
    ...
    expect(numberInput).toContain('aria-invalid={Boolean(validationErrors.number)}')
    expect(numberInput).toContain('data-garage-field="number"')
    expect(peopleInput).toContain('type="number" min="0" max="1000" step="1" required')
    expect(peopleInput).toContain('aria-invalid={Boolean(validationErrors.peopleCount)}')
    expect(floorInput).toContain('type="number" min="0" max="100" step="1" required')
    expect(floorInput).toContain('aria-invalid={Boolean(validationErrors.floorCount)}')
  ...
  it('keeps every report multi-selection on the shared searchable checkbox pattern', () => {
    ...
    expect(reportPanelSource.match(/<ReportCheckboxMultiSelect\b/g)).toHaveLength(6)
    expect(reportPanelSource).toContain('ariaLabel="Фонды отчета"')
    expect(reportPanelSource).toContain('ariaLabel="Гаражи списка"')
  ...
  it('keeps shared select controls consistently styled and labeled without native select rules', () => {
    ...
    expect(financePanelSource).toContain('aria-label="Услуга выплаты поставщику"')
    ...
    expect(financePanelSource).toContain('<LocalizedDatePicker ariaLabel="Месяц поступлений с"')
    expect(financePanelSource).toContain('<LocalizedDatePicker ariaLabel="Месяц поступлений по"')
    expect(financePanelSource).toContain('ariaLabel="Месяц выплат с"')
    expect(financePanelSource).toContain('ariaLabel="Месяц выплат по"')
    ...
    expect(financePanelSource).not.toContain('<select aria-label="Месяц выплат с"')
    expect(financePanelSource).not.toContain('<select aria-label="Месяц выплат по"')
    ...
    expect(normalizedAppCss).not.toContain('.report-workbook-filter__fields > label:first-child .localized-date-picker__popover')
    expect(reportPanelSource).toContain('<details className="report-garage-filter-disclosure" ref={garageFiltersRef} open={garageFiltersOpen}>')
    expect(reportPanelSource).toContain('aria-controls="garage-report-personal-filters"')
    expect(reportPanelSource).toContain('role="region"')
    expect(reportPanelSource).toContain('aria-label="Гаражи и личные фильтры отчёта"')
    expect(reportPanelSource).toContain('className="localized-date-picker__popover report-garage-filter-panel"')
    expect(normalizedAppCss).toMatch(/\.report-garage-filter-panel \{[^}]*width: min\(var\(--report-garage-filter-panel-width, 760px\), calc\(100vw - 48px\)\);[^}]*height: min\(var\(--report-garage-filter-panel-height, 480px\), 78vh\);/u)
    expect(normalizedAppCss).toMatch(/\.report-garage-filter-panel \.payments-prototype-search-results \{[^}]*grid-template-columns: repeat\(auto-fit, minmax\(180px, 1fr\)\);[^}]*width: 100%;/u)
    expect(normalizedAppCss).toMatch(/\.report-checkbox-picker \.payments-prototype-search \{[^}]*display: flex;[^}]*align-items: center;/u)
    expect(normalizedAppCss).toMatch(/\.report-garage-filter-panel \{[^}]*resize: both;/u)
    expect(normalizedAppCss).toMatch(/\.report-garage-filter-disclosure \{[^}]*position: relative;[^}]*z-index: 2;/u)
    expect(normalizedAppCss).toMatch(/\.report-garage-filter-panel \{[^}]*align-content: start;/u)
    expect(normalizedAppCss).toMatch(/\.report-garage-quick-list-actions \{[^}]*align-items: center;/u)
    ...
    expect(normalizedAppCss).toMatch(/@media \(max-width: 720px\) \{[\s\S]*?\.report-garage-filter-panel \{[\s\S]*?resize: none;/u)
  })
