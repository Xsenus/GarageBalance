...
describe('FinancialJournalPanel', () => {
  ...
  it('opens the row menu from the keyboard, validates cancellation, and restores focus', async () => {
    ...
    expect(within(menu).getByRole('menuitem', { name: 'Отменить' })).toHaveFocus()
    await user.keyboard('{End}')
    expect(within(menu).getByRole('menuitem', { name: 'Отменить' })).toHaveFocus()
    await user.keyboard('{Tab}')
    expect(screen.queryByRole('menu', { name: /Действия записи журнала/ })).not.toBeInTheDocument()
    await waitFor(() => expect(row).toHaveFocus())
    fireEvent.keyDown(row, { key: 'ContextMenu' })
    const reopenedMenu = await screen.findByRole('menu', { name: /Действия записи журнала/ })
    await user.click(within(reopenedMenu).getByRole('menuitem', { name: 'Отменить' }))
